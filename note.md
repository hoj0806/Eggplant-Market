...
- **대댓글** — `parent_id`·`comments_parent_idx`가 이미 있고 이번 정책들이 `parent_id`를
  보지 않으므로 그대로 쓸 수 있다. 남은 일은 목록을 트리로 접는 것과, 부모가 지워질 때
  딸린 답글을 어떻게 보일지 정하는 것이다(FK가 `cascade`라 지금은 함께 사라진다).
