...
그 동선에 깔았다.
**(2026-08-10에 배지가 세는 것이 "남아 있는 알림"으로 바뀌면서 이 RPC와 인덱스는 안 쓰게 됐다 —
아래 "모두 읽음을 걷어냈다". 목록을 나눠 쓸 수 없다는 위 이유는 그대로다.)**
...
없는** 상태가 생긴다. (2026-08-10에 배지가 "안 읽은 수"에서 "남은 수"로 바뀌었지만 이 논리는
그대로다 — 세는 자리가 테이블이라는 사실이 같기 때문이다.)
...
버튼 하나를 없앤 일이 **배지의 뜻까지 끌고 왔다.** "모두 읽음"이 있을 때는 배지가 0이 되는
길이 둘이었다(전부 읽거나, 전부 지우거나). 이제 **지우는 것 하나뿐**이다.

그 상태에서 배지가 계속 "안 읽은 수"를 세면 이런 화면이 생긴다.

```
종 배지 0   ·   목록에는 스무 줄이 그대로
```

읽기만 하고 안 지운 알림이 정확히 그렇다. **"0인데 볼 것이 스무 줄"** 은 배지가 거짓말하는
것에 가깝다. 그래서 배지가 세는 것을 **남아 있는 알림**으로 바꿨다 — 지워야 줄어든다.

바뀐 자리가 셋이다.

| 무엇 | 전 | 후 |
|---|---|---|
| 세는 대상 | `is_read = false` | **남아 있는 행 전부** |
| 알림을 눌렀을 때 | 하나 깎음 | **안 건드림**(줄이 남아 있으니까) |
| 한 줄 지웠을 때 | 안 읽었던 것만 깎음 | **언제나 하나 깎음** |

`is_read`를 버린 것이 아니다. 목록의 **굵은 글씨와 "안 읽음" 표식은 그대로**다 — 줄 단위로
"봤나"를 말하는 자리와 배지가 "얼마나 쌓였나"를 말하는 자리는 다른 질문이다.

#### RPC를 안 쓰고 평범한 count로 센다

`count_unread_notifications()`는 `where is_read = false`가 박혀 있어 못 쓴다. 새 RPC를 만드는
대신 **평범한 count**로 옮겼다 — 세는 데 join도 payload 풀기도 필요 없고, **범위는 RLS가
정한다**(삭제 쪽과 같은 약속이다). `head: true`라 행을 안 받고 숫자만 받는다.

그 요청이 **실제로 되는지는 코드를 읽어서 알 수 없어** 통합 테스트로 확인했다
(익명이 세면 오류 없이 0). 조건 없는 delete를 확인한 것과 같은 자리다.

**옛 RPC와 부분 인덱스는 아직 안 지웠다.** 배포된 `main`이 여전히 그것을 부르기 때문이다 —
지금 지우면 살아 있는 화면의 배지가 깨진다. `backlog.md`에 적어 두고 배포가 따라온 뒤에 지운다.

### 남는 결과 하나

**배지를 0으로 내리는 길은 이제 "모두 삭제"뿐이다.** 읽는 것으로는 안 줄어든다.
읽지 않은 채 배지만 지우고 싶은 상황이 있을 수 있지만, 그것을 위해 "눌러도 목록이 그대로인
버튼"을 두는 값보다 작다고 봤다.

### 확인

- `npm test` **930개** · lint · tsc 통과
  (`withAllNotificationsRead` 테스트가 빠지고 배지 count 통합 테스트가 들어왔다)
- 화면에서 눈으로 보고 내린 판단이다. **시드가 없었으면 못 봤을 자리다** —
  알림이 0줄이면 버튼도 배지도 안 뜬다.
