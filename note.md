...
- **인기순의 정의** — 조회수 하나로 잡았다. 조회·찜·채팅을 섞은 점수는 기준을 정하는 일이지
  거는 일이 아니라 따로 다룬다.
  → **2026-08-10에 그대로 확정됐다**: 인기순은 조회순이다. 섞은 점수는 안 만든다.
  여기서 "따로 다룬다"고 미뤄 둔 것이 결국 **처음 잡은 대로**가 됐고, 코드는 한 줄도
  안 고쳤다(`backlog.md` §5-3). 라벨 "조회 많은 순"도 그대로 둔다 — 정의가 조회수라면
  그렇게 적는 편이 "인기순"보다 정확하다.
...
> **뒤에 정해진 것 (2026-08-10)**: 신고를 **처리하는 화면은 안 만들기로 했다**
> (`backlog.md` §5-7). 신고는 쌓이되 아무도 읽지 않는다 — 읽을 일이 생기면 대시보드 SQL이
> 먼저다. 그러면서 완료 문구가 **"운영팀이 확인한 뒤 조치합니다"** 라고 없는 일을 약속하고
> 있던 것을 고쳤다. 지금은 **실제로 효과가 있는 것(차단)** 만 말한다.
...
1. **`/notifications`** — 알림 목록. 무한 스크롤, 안 읽은 줄 표시, "모두 읽음".
   (~~안 읽은 줄 표시~~ · ~~"모두 읽음"~~ 둘 다 2026-08-10에 없앴다 — 맨 아래 "읽음을 없앴다".)
...
그 동선에 깔았다.
**(2026-08-10에 배지가 "남은 알림 수"를 세게 되면서 RPC도 인덱스도 지웠다(0036).
목록을 나눠 쓸 수 없다는 위 이유는 그대로다 — 평범한 count로 옮겼을 뿐이다.)**
...
**(2026-08-10에 읽음 상태를 통째로 없애면서 이 문단이 말하는 두 경로가 다 사라졌다.
남은 것은 "누르면 지워진다" 하나다 — 맨 아래 절.)**
...
4. **배지 동기화** — 안 읽은 알림을 지우면 숫자도 하나 줄어든다.
   (2026-08-10부터는 **어떤 줄이든** 지우면 하나 준다. 배지가 남은 수를 세기 때문이다.)
...
> 이 판단은 **한 줄 삭제에만** 남았다. "모두 삭제"(아래 후속)는 한 번 더 묻고,
> 2026-08-10에 **누르는 것도 삭제가 되면서** 묻지 않는 자리가 하나 더 늘었다.
...
- ~~**알림 전체 삭제**~~ — ✅ 아래 후속(2026-08-10)에서 만들었다. 여기 적어 둔 예상
  ("확인 절차가 따라오고, 그러면 4번 판단을 다시 해야 한다")이 **그대로 맞았다.**
...
`backlog.md` §4의 알림 잔여 하나. **마이그레이션이 없다** — 0019의
`notifications_delete`(`using (auth.uid() = user_id)`)가 행 개수를 묻지 않으므로
한 줄을 지우는 정책이 전부를 지우는 정책이기도 하다. 그래서 이번 일은 처음부터 끝까지
**"정책이 이미 허락한 일을 화면이 어떻게 물어볼 것인가"** 였다.

#### 1. 위 4번 판단이 뒤집힌다 — 여기서는 묻는다

한 줄 삭제에 확인을 두지 않은 근거는 **"잘못 눌러도 잃는 것이 알림 한 줄뿐"** 이었다.
그 근거가 그대로 뒤집힌다. 한 번의 실수로 전부 사라지고, 위에 적은 대로 **되돌릴 방법이
아예 없다.** 판단이 바뀐 것이 아니라 **같은 기준을 다시 재니 답이 달라진 것**이다.

확인의 **모양**은 채팅방 나가기·차단과 같게 했다(제자리에서 문구 + 실행/취소).
탈퇴처럼 문구를 받아 적게 하지는 않았다 — 사라지는 것이 알림뿐이고 그것이 가리키던
대화·글·후기는 그대로 남는다. **확인의 무게는 잃는 것의 무게를 따라간다.**

문구에 "안 읽은 알림까지 함께 사라진다"를 적었다. 목록에서 굵은 글씨는 보이지만
**아래쪽 페이지에 무엇이 남아 있는지는 누르는 사람이 모른다.**

#### 2. 배지는 세지 않고 0으로 둔다

한 줄 삭제는 `withDecrementedUnread`로 하나씩 깎았는데 여기서는 안 쓴다.
**몇 개가 안 읽은 것이었는지 화면이 알 수 없기 때문이다** — 목록은 첫 페이지만 받아 온
상태일 수 있고 배지는 전체를 센다. 다 지웠으면 안 읽은 것도 0이다("모두 읽음"이 0을
그대로 쓰는 것과 같은 자리).

#### 3. 캐시는 페이지를 비우되 **페이지 배열은 남긴다**

`withoutAllNotifications`는 각 페이지를 빈 배열로 바꾸고 개수는 그대로 둔다.
`withoutNotification`이 그렇게 한 이유(페이지 배열은 커서의 흔적)에 여기서는 값이 하나 더 붙는다.

```
toNextNotificationCursor: 마지막 페이지가 덜 찼으면 → undefined(다음 없음)
빈 배열은 언제나 덜 찬 페이지 → hasNextPage가 저절로 꺼진다
```

**두 함수는 서로를 모르지만 함께 맞아야** 지운 뒤 목록이 조용해진다. 그 맞물림을 테스트가
직접 밟는다(`withoutAllNotifications` → `toNextNotificationCursor`가 `undefined`).
무효화로 갈음하지 않은 이유는 왕복 때문이다 — 결과를 이미 아는 요청이다.

#### 4. 조건 없는 delete를 실제 DB에 대고 지킨다

`deleteAllNotifications`는 사실상 조건이 없다. 범위를 정하는 것이 **정책 하나뿐**이라,
정책이 흔들리면 남의 알림까지 지워지는 종류의 코드다. 그래서 통합 테스트를 붙였다
(`anonymousWriteGuards.int.test.ts`).

```
익명 클라이언트로 조건 없는 delete → 오류 없음(0건 삭제도 성공이다)
                                  → 심어 둔 남의 알림 1건 그대로
```

**두 가지를 함께 본다.** PostgREST가 조건 없는 delete를 **받는다**는 것(안 받으면 화면이
조용히 실패한다)과, 그럼에도 **한 줄도 안 지워진다**는 것. 앞쪽은 코드를 읽어서는 알 수
없고 실제로 보내 봐야 안다 — 붙이기 전에는 거절당할 가능성을 반반으로 봤다.

`user_id`로 한 번 더 좁히지 않은 것은 `markAllNotificationsRead`와 같은 이유다.
대신 조건 자리에 `id is not null`(언제나 참)을 적었다 — 정책을 못 믿어서가 아니라
**조건 없는 delete가 실수처럼 읽히기 때문**이다.

#### 5. 버튼을 내는 기준이 "모두 읽음"과 다르다

"모두 읽음"은 **안 읽은 것이 있을 때**, "모두 삭제"는 **한 줄이라도 있을 때** 나온다.
다 읽은 목록도 치우고 싶을 수 있다 — 오히려 그쪽이 치우고 싶은 목록이다.

> **뒤에 일어난 일 (2026-08-10)**: 화면에서 나란히 놓고 보니 "모두 읽음"이 애매해서 없앴고,
> 이어서 **읽음이라는 상태 자체**를 걷어냈다. 그래서 지금 머리말에 서는 버튼은 하나뿐이다.
> 맨 아래 "알림에서 읽음을 없앴다".

#### 확인

- `npm test` **930개**(유닛 843 + 통합 87) · lint · tsc 통과
- 새 테스트: `withoutAllNotifications` 4개(커서 맞물림 포함) ·
  `deleteAllNotificationsButton` 4개(확인 없이는 안 지운다 · 취소 · 캐시와 배지 · 실패) ·
  통합 1개(위 4번)
- ✅ **눈으로 확인했다** (2026-08-10). (그날 밀려 있던 아홉 건을 한 번에 훑었다.)
- `notificationPage`가 버튼을 내고 감추는 조건(5번)에는 테스트가 없다.
  그 화면에 테스트 하네스가 아직 없고, 조건 하나 때문에 세우지 않았다.
...
배지는 테이블을 세지 목록 RPC를 세지 않는다. 감추기만 하면 **배지는 3인데 목록에는 한 줄도
없는** 상태가 생긴다. (2026-08-10에 배지가 세는 것이 "남은 수"로 바뀌었어도 이 논리는 그대로다 —
세는 자리가 테이블이라는 사실이 같다.)
...
~~화면은 아직 눈으로 못 봤다~~ — ✅ **눈으로 확인했다** (2026-08-10). 그때까지는 서버 규칙과 테스트까지였다.
...
~~화면은 아직 눈으로 못 봤다~~ — ✅ **눈으로 확인했다** (2026-08-10). 그전까지 체크칸·"비밀" 표·안내 한 줄은 테스트로만 확인한 상태였다.
...
- ~~화면은 여전히 눈으로 못 본 것이 여덟 건이다~~ — ✅ **눈으로 확인했다** (2026-08-10). 아홉 건이 되었다가 한 번에 닫혔다(`design.md` §8).
...
"성능에 문제가 있을 것 같으면 최적화하고, 최적화한 것은 반드시 기능 테스트를 붙일 것.
모든 기능을 최적화할 필요는 없고 꼭 필요한 데만." — 그래서 **먼저 재고 나서 두 곳만 고쳤다.**

마이그레이션 없음. 테스트 32개가 늘어 **828개**가 됐다.

### 재는 것부터 했다

데이터가 6건뿐이라 "느리다"를 증명할 수 없었다. 임시 계정 하나로 **게시물 2만 건**을
서울 전역 200개 동네에 흩뿌리고 실제 경로를 쟀다(끝나고 계정 하나 지우니 cascade로 전부 사라졌다).
...
**DB는 병목이 아니다.** 그래서 인덱스도 정책도 손대지 않았다.

#### 헛다리 두 번 (`troble.md`에 자세히)

처음 잰 값은 405ms, 그다음은 111ms였다. 둘 다 **측정이 틀린 것**이었다 —
2만 건을 전부 반경 2km 안에 몰아넣었고(그러면 Seq Scan이 옳다), 그다음엔 연결마다
처음 한 번 드는 카탈로그 적재 비용을 재고 있었다. 고칠 뻔했다.

### 고친 것 1 — 업로드 전에 사진을 줄인다

**이 앱의 가장 큰 성능 문제였다.** 폰 사진 5MB가 원본 그대로 올라가고, 홈 목록은 그 원본을
96px 썸네일 자리에 그린다. 카드 스무 개면 **최대 100MB를 내려받는다.**

`shared/utils/downscaleImage.ts` — 긴 변 1600px, JPEG 품질 0.8. 게시물·프로필·채팅 사진
세 경로에 모두 붙였다.

정한 것 다섯:

1. **서버가 아니라 브라우저에서 줄인다.** Supabase 이미지 변환은 유료 플랜이고, Edge Function으로
   흉내 내면 **올리는 사람의 데이터는 여전히 원본만큼** 든다. 브라우저에서 줄이면 그것까지 준다.
2. **원본보다 키우지 않는다.** 작은 사진을 1600px로 늘리면 용량만 커진다.
3. **GIF는 손대지 않는다.** 캔버스에 그리면 움직이는 그림이 첫 장면으로 납작해진다.
4. **줄였는데 더 커지면 원본을 쓴다.** 이미 잘 압축된 JPEG를 다시 인코딩하면 실제로 커진다.
5. **실패하면 원본으로 물러난다.** 여기서 예외를 던지면 "사진을 줄이지 못해서 글을 못 올리는"
   일이 생긴다. 그건 고치려던 것보다 나쁘다.

투명한 PNG를 JPEG로 바꾸면 투명한 자리가 검게 남아서, 그리기 전에 흰 배경을 깐다.

### 고친 것 2 — 목록 사진은 화면에 들어올 때 받는다

`loading="lazy"`가 **한 곳도 없었다.** 붙인 곳과 안 붙인 곳을 갈랐다.

| 붙임 | 안 붙임 |
|---|---|
| 목록 카드 · 채팅방 목록 · 프로필 사진 · 채팅 사진 | **상세 화면 대표 사진** · 채팅방 상단 |

규칙 하나다 — **보이는 것을 먼저 준다.** 상세의 대표 사진은 그 화면에서 가장 크고 먼저 보이는
것(LCP)이라 늦추면 "사진이 늦게 뜨는 앱"이 된다. 목록에서 아끼는 이유와 정확히 같은 이유로
여기서는 안 아낀다. **그 규칙이 조용히 뒤집히지 않도록 테스트로 박아 두었다**
(`postImageCarousel.test.tsx`의 "지연 로딩을 붙이지 않는다").

### 테스트 32개

- `downscaleImage.test.ts` (17) — 비율·상한·GIF 제외·이름 바꾸기, 그리고 **물러나는 네 갈래**
  (작은 사진 · 더 커짐 · 그리기 실패 · 읽기 실패). 캔버스가 없는 jsdom을 위해 그리는 경계를
  갈아끼운다(`kakaoMapLoader`와 같은 구조).
- `postCard.test.tsx` (6) · `profileAvatar.test.tsx` (4) — 지연 로딩이 붙었는가 + 기존 동작.
- `postImageCarousel.test.tsx` (5) — **안 붙었는가** + 넘김 동작.

### 안 한 것과 그 이유

- **번들 쪼개기** — 797 kB(gzip 224 kB) 한 덩어리다. 경고가 뜨지만 react·supabase·router가
  대부분이라 라우트를 쪼개도 첫 로드가 크게 줄지 않는다. **사진 100MB를 고친 뒤에 224 kB를
  붙드는 것은 자리를 잘못 고르는 일이다.**
- **인덱스 정리** — 0회 스캔 4개와 완전 중복 2개(`posts_region_code_idx`는
  `posts_region_keyset_idx`의 앞부분)를 찾았지만, 쿼리가 이미 0.5ms라 얻을 것이 없다.
- **RLS `(select auth.uid())` 감싸기** — 어드바이저가 26건 권했다. 행마다 다시 재는 것을
  한 번으로 줄이는 표준 처방인데, 지금 규모에서 잴 수 있는 차이가 없어 미뤘다.
- **TanStack Query 설정** — `staleTime` 60초가 이미 붙어 있었다.

### 이번 범위 밖

- **`browserDownscaler`는 자동 테스트가 못 닿는다.** jsdom에 캔버스가 없다. 실제 축소는
...
## 파비콘 — 탭에 가지를 띄운다 (2026-08-09)

`plan-2026-08-09.md` ①. 탭에 기본 지구본이 뜨던 것을 고쳤다.
...
`index.html`이 그대로 왔다. `public/` 폴더가 아예 없었고, `vercel.json`의 SPA 폴백이
**없는 파비콘 요청까지 가로챘다.** 200이라 실패로 보이지도 않았다 — 폴백이 있는 SPA에서는
**404를 못 보는 것이 정상**이라 "파일이 없다"는 신호가 어디에도 안 뜬다.

### 가지를 두 벌로 만들지 않으려고

`brandMark.tsx`에 이미 직접 그린 가지가 있었다. 파비콘에 같은 그림이 필요한데,
SVG 파일은 정적이라 **TS를 import할 수 없다.** 값을 옮겨 적는 수밖에 없고, 옮겨 적은 사본은
언젠가 한쪽만 고쳐진다.

그래서 좌표와 색을 `shared/ui/brandMarkShape.ts` 한 곳에 모으고,
`public/favicon.svg`가 그 값과 같은지를 **`brandMarkShape.test.ts`가 지키게** 했다.
사본을 없앨 수는 없으니 **어긋나는 순간 깨지게** 한 것이다.

### 화면과 다르게 둔 것 둘 — 둘 다 16px 때문이다

경로(실루엣)는 똑같이 두고 두 가지만 바꿨다.

| | 화면 | 파비콘 | 왜 |
|---|---|---|---|
| 줄기 굵기 | 1.6 | 2.4 | 24분의 1.6은 16px에서 **한 픽셀 남짓**이라 안티에일리어싱에 먹혀 사라진다 |
| 뷰박스 | `0 0 24 24` | `1.8 1.15 20.4 20.4` | 표식은 가로로 24 중 14.2만 쓴다. 화면에선 여백이지만 탭에선 **버리는 픽셀**이다 |

뷰박스는 **도형의 실제 경계를 정사각형으로 맞춘 값**이다. 다른 그림이 아니라 같은 그림을
바짝 자른 것이라, 실루엣은 그대로다.

### .ico를 만든 방법

`.ico`는 바이너리라 손으로 못 쓰고, 저장소에 SVG 래스터라이저가 없다(ImageMagick도 sharp도
없다). 가지는 베지어 몇 개뿐이라 도구를 들일 무게가 아니어서
**`scripts/generateFavicon.py`가 `favicon.svg`를 읽어 직접 래스터라이즈**한다.
Pillow만 있으면 되고, 표식이 바뀔 때만 돌린다.

두 가지를 일부러 손으로 했다.

- **크기마다 따로 그린다.** Pillow의 ICO 저장은 한 장을 받아 알아서 줄이는데,
  그러면 16px이 "뭉갠 32px"이 된다. 16·32·48을 각각 8배로 그려 줄인 뒤
  **ICO 컨테이너를 직접 썼다.**
- **PNG를 담은 ICO**로 만들었다(Vista 이후 형식). 요즘 브라우저는 전부 읽는다.
...
브라우저는 **읽을 수 있는 것 중 뒤에 온 것**을 고른다. 그래서 ico가 앞이다 —
뒤집으면 SVG를 못 읽는 쪽이 svg를 집었다가 아무것도 못 그린다.
그리고 **링크가 있어도 `.ico` 파일 자체는 있어야 한다.** 브라우저는 링크와 별개로
`/favicon.ico`를 찔러 보기 때문이다.

### `vercel.json`은 안 고쳤다

Vercel은 **정적 파일을 rewrite보다 먼저 매칭**한다. `vite preview`로 같은 순서를 확인했다.

```
/favicon.svg  ->  200  image/svg+xml   (876 B)
/favicon.ico  ->  200  image/x-icon    (4056 B)
/posts/1      ->  200  text/html       (폴백은 그대로)
```

**배포 후 같은 curl을 다시 해야 한다.** `text/html`이 오면 rewrite에 제외 규칙을 더한다.

---

## 링크 공유 카드 — 메타 태그와 og 그림 (2026-08-09)

`plan-2026-08-09.md` ⑤. `<head>`에 `charset`·`viewport`밖에 없어서 **카카오톡이나 슬랙에
주소를 붙이면 제목도 설명도 썸네일도 안 나왔다.** 중고거래 앱에서 링크 공유는 실제 사용
경로라 그냥 둘 자리가 아니다.

### 절대 주소여야 하는 것 하나

계획에는 `og:image`를 `/og-image.png`로 적어 두었는데 **절대 주소로 바꿨다.**
페이스북은 상대 경로를 알아서 붙여 주지만 **슬랙·카카오를 포함한 여러 수집기는 그대로 두고
그림을 못 찾는다.** Open Graph 명세가 절대 URL을 요구하는 쪽이라 맞추는 편이 잃을 것이 없다.

그래서 배포 주소가 **코드 안에 처음 들어왔다.** 지금까지는 문서에만 있었다.
`shared/ui/siteMeta.ts`에 두었고, 이유가 하나 더 있다 — **계획 ⑧B 스모크 테스트가 같은
주소를 쓴다.** 그때 세 번째 사본을 만들지 않으려고 미리 한 곳에 모았다.

`SITE_URL`은 끝에 슬래시를 안 붙인다(뒤에 경로를 이어 붙인다). 그 규칙도 테스트가 지킨다 —
슬래시가 붙는 순간 `//og-image.png`가 되기 때문이다.

### 파비콘에서 쓴 방법을 그대로 썼다

`index.html`은 정적이라 `siteMeta.ts`를 import할 수 없다. `brandMarkShape.ts`와 똑같은
사정이라 똑같이 했다 — **값을 옮겨 적되 `siteMeta.test.ts`가 어긋나면 깨뜨린다.**

태그를 찾는 정규식이 한 가지를 더 감당해야 했다. prettier가 긴 `content`를 여러 줄로
내려 쓰기 때문에 `<meta\n  property="og:title"\n  content="…"\n/>` 모양이 된다.
`[^>]*`가 줄바꿈을 포함하므로 태그 하나 안에서만 매칭되고, 속성 순서가 뒤집힌 경우까지
두 갈래로 잡는다.

### og 그림은 표식을 다시 그리지 않았다

`scripts/generateOgImage.py`가 **`generateFavicon.py`의 래스터라이저를 import해서** 쓴다.
그래서 `public/favicon.svg`가 탭 아이콘과 공유 카드 **양쪽의 유일한 원본**이다.
가지가 세 벌이 되는 것을 막았다.

1200 x 630 (1.91:1) — 페이스북·슬랙·카카오가 모두 이 비율로 자른다. 25 kB.
흰 배경 위에 옅은 보라 판을 깔았는데, **카카오톡 흰 말풍선 위에서 카드 경계가 안 보이기**
때문이다.

테스트가 PNG 머리말의 IHDR을 직접 읽어 **`og:image:width`에 적은 숫자와 실제 파일 크기가
같은지** 본다. 숫자만 고치고 그림을 안 바꾸는 사고를 잡는 자리다.

### 게시물별 카드는 안 만든다

`backlog.md` §4에 적었다. 요지는 **카카오톡·슬랙 봇이 JS를 실행하지 않는다**는 것이다 —
메타를 자바스크립트로 바꿔 봐야 소용없고, SSR이나 프리렌더가 따라온다.

### 확인

- 유닛 787개 통과(새 11개 포함) · lint · tsc · build 통과
- `dist/`에 `og-image.png`가 실려 나가고 `/og-image.png` → `image/png` (`vite preview`)
- `dist/index.html` 3.74 kB · gzip 1.83 kB

**밖에서 못 재는 것**: 카드가 실제로 뜨는지는 카카오·슬랙이 직접 긁어가 봐야 안다.
배포 후 카카오톡 나에게 보내기로 한 번 확인한다. 파비콘 때와 같은 종류의 한계다.

---

## 빌드 산출물을 테스트한다 — jest 세 번째 갈래 (2026-08-09)

`plan-2026-08-09.md` ⑧A. 테스트 849개는 **소스가 맞는가**를 묻지 **빌드 결과물은 한 번도
안 본다.** ts-jest가 TypeScript를 CommonJS로 바꿔 돌리는 것이라 `dist/`를 읽는 테스트가
하나도 없었다. 그 틈으로 2026-08-08 하루에만 셋이 샜다.

### 왜 갈래를 나눴나

`jest.config.cjs`에 `build`를 더했다. `*.build.test.ts` · `testEnvironment: node` ·
**소스가 아니라 `dist/`를 읽는다.**

`unit`·`integration`을 나눈 이유는 **환경**이었지만(jsdom이냐 아니냐, 클라이언트를 갈아끼우냐),
`build`를 나눈 이유는 다르다. **묻는 것이 다르다.**

```
unit · integration   로직이 맞는가
build                제대로 실려 나갔는가
```

그래서 **`npm test`에서 뺐다**(`jest --selectProjects unit integration`).
`npm run test:build`가 `npm run build`부터 하고 돌린다. 섞어 두면 낡은 `dist/`를 검사한
초록이 나오는데, **그 초록은 지금 코드에 대한 말이 아니다.**

`unit`의 `testPathIgnorePatterns`에도 `\.build\.test\.tsx?$`를 더해야 했다 —
안 그러면 jsdom 갈래가 빌드 테스트를 주워 가서 `npm test`가 이유 없이 빨개진다.

### 값이 아니라 형식을 본다

**저장소에 정답이 없다.** `.env.local`은 커밋되지 않고, 배포는 Vercel 대시보드의 값을 쓴다.
그래서 "번들의 값이 X와 같은가"는 물을 수 없다. 대신 **형식과 길이**를 본다.

| 무엇 | 검사 |
|---|---|
| Supabase 주소 | `https://[a-z0-9]{20}\.supabase\.co` |
| 공개 키 | `sb_publishable_` + 20자 이상 (옛 JWT 키면 걸린다) |
| 카카오 앱키 | 32자 hex 문자열 리터럴 |

**한 글자 잘린 키는 형식에서 걸린다** — 어제 사고를 정확히 잡는 자리다.

### 카카오 앱키를 찾는 데 한 번 막혔다

`appkey=` 뒤에서 찾으려 했는데 **안 나온다.** 소스는 템플릿 문자열인데
(`?appkey=${KAKAO_APP_KEY}`) 번들에서는 키가 변수로 빠지기 때문이다.
...
그래서 **32자 hex 문자열 리터럴**을 찾는 쪽으로 바꿨다. 그런데 supabase-js에
`"00000000000000000000000000000000"`(0이 32개)이 들어 있어서 그것도 걸린다 —
**같은 글자만 반복되는 것은 뺀다.** 앱키와 SDK 주소는 따로 확인한다(앱키가 있어도
로더가 빠지면 지도가 안 뜬다. 다른 사고다).

### `sb_secret_` 오탐

계획이 미리 적어 둔 대로다. **`sb_secret_` 문자열 자체가 supabase-js의 형식 판별 코드에
정상적으로 들어 있다.** 접두사만 찾으면 매번 빨개진다.

```
sb_secret_            번들에 1건 (정상)
sb_secret_[\w-]{10,}  0건        ← 이쪽만 잡는다
```

JWT도 같은 이유로 **토막 셋을 요구한다.** `eyJ`만 찾으면 base64로 인코딩된 아무 데이터나 걸린다.

### 그물이 실제로 걸리는지 확인했다

**초록만 보고 끝내면 안 되는 종류의 코드다.** 통과하는 것은 아무것도 증명하지 않는다 —
검사가 아예 안 도는 경우에도 초록이기 때문이다. 그래서 `dist/`를 일부러 망가뜨려
빨개지는지 하나씩 봤다.

| 망가뜨린 것 | 결과 |
|---|---|
| 앱키에서 한 글자 뗌 (어제 사고 재현) | ✅ 실패 |
| `sb_secret_` + 실제 키 주입 | ✅ 실패 |
| `md:grid-cols-2` 규칙 제거 | ✅ 실패 |
| `.page-narrow` 규칙 제거 | ❌ **통과했다** |

마지막 것이 수확이었다. `toContain(".page-narrow")`는 **`.page-narrowX`도 통과시킨다** —
이름이 바뀌어 규칙이 사라진 경우를 못 잡는다. 선택자 뒤에 `{`·`,`·`:`·공백 중 하나를
요구하도록 고쳤다. **테스트를 검사하지 않았으면 그대로 남았을 구멍이다.**

### 낡은 `dist/`를 검사하지 않는다

`dist/index.html`의 mtime이 소스보다 이르면 **실패시킨다.** 2026-08-08에 환경변수를 고치고
재배포를 안 해 옛 번들이 계속 서비스된 일이 있었는데, 같은 모양의 착각을 로컬에서 먼저 막는다.

만들고 나서 바로 걸렸다 — **빌드 테스트 파일을 고칠 때마다 뜬다.** 틀린 말은 아니지만
빌드에 안 들어가는 파일이고, 잔소리가 잦으면 사람은 검사를 꺼 버린다.
**`*.test.ts(x)`는 세지 않는다.** 둘 다 확인했다(소스 고치면 뜬다 · 테스트만 고치면 안 뜬다).

### 지금 상태

- `npm test` — **849개** (unit + integration)
- `npm run test:build` — **35개** (build)
- lint · tsc · build 통과

### 이 갈래가 못 보는 것

**실제로 서비스되는 것.** 빌드가 맞아도 배포가 틀릴 수 있다. 그쪽은 배포 주소를 받아
검사하는 스모크(계획 ⑧B)의 몫이고, 브라우저가 있어야 아는 것(캔버스 축소·Realtime·
로그인 플로우)은 **여전히 사람 눈에 남는다.**

---

## 배포된 사이트 스모크 — jest 네 번째 갈래 (2026-08-09)

`plan-2026-08-09.md` ⑧B. `build`가 *"제대로 만들어졌는가"*라면 여기는
*"제대로 서비스되는가"*다.

### 겹치는 검사를 일부러 두 번 한다

`build`와 `smoke`가 같은 것을 본다 — 환경변수가 박혔는가, 비밀이 안 섞였는가.
겹쳐 보이지만 **빌드가 맞아도 배포가 틀릴 수 있다.** 2026-08-08에 환경변수를 고치고
재배포를 안 해 옛 번들이 계속 나갔는데, **그때 `dist/`는 내내 옳았다.**

> **`smoke`가 빨간데 `build`가 초록이면 코드가 아니라 배포가 밀린 것이다.**

다만 **규칙까지 두 벌이면** 한쪽만 고쳐지는 날이 온다. 정규식과 추출 함수를
`shared/testUtils/bundlePatterns.ts` 한 곳에 모으고 두 갈래가 나눠 쓴다.
(⑧A에서 `distAssets.build.test.ts`에 적어 두었던 것을 옮겼다.)

### 만들자마자 제 일을 했다

프로덕션은 `main`에서 배포되는데 **`develop`이 12커밋 앞서 있었다.** 그래서 첫 실행에서
**21개 중 7개가 빨갛게** 나왔다 — 전부 파비콘·og 관련이다.

```
✅ SPA 폴백 · 자산 캐시 헤더 · 배포된 번들의 환경변수/비밀 · 카카오 SDK · auth 설정
❌ /favicon.svg · /favicon.ico · /og-image.png  → 전부 text/html
❌ 배포된 index.html의 head에 파비콘·og 태그 없음
```

**이것이 정확히 잡으라고 만든 실패다.** 로컬 `build` 35개는 전부 초록인 채로.

### 검사가 무엇을 재는지 확인하는 검사

카카오 SDK 검사는 **`Referer`를 배포 주소로 바꿔 요청해 200인지**를 본다.
그런데 그것만으로는 부족하다 — **카카오가 아무 Referer나 받아 준다면** 그 초록은
"도메인이 등록됐다"가 아니라 "카카오 서버가 살아 있다"는 뜻이 된다.

그래서 **등록 안 한 주소로도 한 번 부른다.** 막히는 것까지 봐야 앞의 200이 의미를 갖는다.
⑧A에서 `dist/`를 일부러 망가뜨려 본 것과 같은 생각이다.

### 주소가 없으면 실패시킨다

건너뛰는 스모크는 **초록인 채로 아무것도 안 지킨다.** 주소가 없으면 그물이 없는 것보다
나쁘다 — 있다고 믿게 만들기 때문이다. 그래서 `smokeTarget()`이 던지고,
`scripts/runSmoke.mjs`도 종료코드 1로 끝난다.

### 리다이렉트를 따라가지 않는다

`fetch`에 `redirect: "manual"`을 준다. 따라가면 **Vercel 배포 보호의 로그인 화면을
200으로 받아 들고 "잘 뜬다"고 착각한다.** Preview 배포를 확인하려다 알게 됐다 —
Preview는 전부 302로 막혀 있어서 **밖에서 잴 수 있는 것은 프로덕션뿐이다.**

계획 ⑥(Preview에서 로그인이 막힌다)도 여기서 정리됐다. 로그인만 막히는 것이 아니라
**Preview 자체가 열리지 않는다.** 와일드카드 등록을 고민할 이유가 없어졌다.

### 지금 상태

- `npm test` — **849개** (unit + integration)
- `npm run test:build` — **35개**
- `npm run test:smoke -- <주소>` — **21개** (프로덕션 기준 14 통과 · 7 실패)
- lint · tsc · build 통과

**7개는 `develop`을 `main`으로 올려 재배포하면 초록이 된다.** 그 배포가 `plan` ①-4가
남겨 둔 "배포 후 Content-Type 확인"이기도 하다 — **이제 curl 대신 이 21개가 대신 본다.**

---

## 방이 먼저 사라진 채팅 사진을 줍는다 (2026-08-09)

`plan-2026-08-09.md` ②·`backlog.md` §1-3. 어제 탈퇴 검증에서 **재현 조건까지 확인한 실제
버그**를 고쳤다.

### 무엇이 문제였나

`chat-images`의 경로는 `{room_id}/{user_id}/…`다(0008). 지울 경로를 아는 **유일한 근거가
`chat_rooms` 행**인데, 그 행이 **남의 탈퇴에 딸려 먼저 사라진다.**
...
0031이 미뤄 둔 "상대가 올린 사진이 남는다"보다 한 겹 더 나쁘다 — **자기 사진인데도** 못 지운다.

### 그물을 하나 더 건다

방 기록 기반 훑기를 **없애지 않았다.** 그쪽이 훨씬 싸고, 대부분의 탈퇴는 방이 살아 있는
상태에서 일어난다. 버킷 훑기는 **못 찾은 나머지를 줍는 두 번째 그물**이다.

```
① 방 기록으로 훑는다        정확하고 싸다. 아는 방은 빠짐없이 본다
② 버킷 최상위를 훑는다       ①이 못 본 방을 줍는다. 상한이 있다
```

②는 ①이 이미 본 방을 건너뛴다. 상한(500)을 넘으면 **조용히 멈춘다** —
**뒷정리가 못 끝나도 탈퇴 자체는 성공해야 한다.** 남는 파일은 아무도 못 읽는 쓰레기라
다음 탈퇴가 지나가며 치워도 된다.

**정책은 안 고쳤다.** `chat_images_delete`에 이미 「방이 없으면 삭제 허용」 조항이 있었다
(0032). 만든 사람이 이 경우를 예상은 했던 자리다. 게다가 여기서 부르는 것은 service_role이라
정책을 지나간다.

### 어떻게 테스트했나 — Deno 코드를 Jest로 검증하기

`index.ts`는 **Deno 런타임과 `npm:` 지정자에 묶여 있어 Jest가 못 읽는다.** 그런데
"어느 폴더를 훑고 무엇을 지울지"는 순수 계산이라 테스트할 수 있어야 하는 부분이다.

그래서 저장소 접근을 **함수 둘로 받게** 떼어 냈다(`ListPage`·`RemovePaths`).
Deno는 진짜 클라이언트를, 테스트는 가짜를 끼운다. **클라이언트 타입에 기대지 않으므로**
`npm:@supabase/supabase-js@2`(Deno)와 `@supabase/supabase-js`(Node)가 같은 모듈을 쓴다.

검증을 둘로 나눴다.

| 무엇을 재나 | 어디서 |
|---|---|
| **판단** — 어느 폴더를 훑고 무엇을 지울지 | `chatImageSweep.test.ts` (가짜 저장소, 20개) |
| **저장소** — 실제로 그렇게 생겼는가 | `chatImageSweep.int.test.ts` (진짜 버킷, 5개) |

**통합 테스트에서 스토리지를 처음 만졌다.** 훑기가 기대는 성질 셋이 전부 Supabase Storage의
동작이라 코드만 봐서는 알 수 없다.
...
방 행이 없는 상황은 **존재하지 않는 방 번호**로 파일을 올려 만들었다.

### 확인에 서비스 키를 쓴 이유

다른 통합 테스트는 "안 보여야 할 것이 안 보인다"를 보려고 **익명 클라이언트로 확인**한다.
여기서는 다르다 — 재는 것이 정책이 아니라 **뒷정리가 파일을 지웠는가**이고, 무엇보다
`chat_images_select`가 방 행을 요구해 **고아 파일은 원래 아무도 못 읽는다.**
익명으로는 지워졌는지조차 확인할 수 없다.

### 딸려 고친 것 — 순수 모듈이 타입 검사를 안 받고 있었다

`tsconfig.json`의 `include`가 `["src", "vite.config.ts"]`뿐이라 **`supabase/` 아래는
`tsc`가 한 번도 안 본다.** ts-jest도 `isolatedModules: true`라 타입을 안 본다 —
즉 새 모듈은 **Jest가 돌리는데 타입 검사는 아무도 안 하는 상태**였다.

`chatImage*`만 `include`에 넣었다. `index.ts`는 Deno 전역과 `npm:` 지정자를 쓰므로
넣으면 `tsc`가 죽는다.

### 남은 일

**`supabase functions deploy delete-account`가 필요하다.** 배포 전까지 프로덕션은
옛 함수(v2)를 그대로 쓴다. 여기는 Vercel과 달리 **git 머지로 안 나간다.**

### 확인

- `npm test` **874개** (기존 849 + 판단 20 + 저장소 5)
- `npm run test:build` 35개 · lint · tsc 통과

---

## 스토리지 뒷정리 경로를 전부 밟았다 (2026-08-09)

`plan-2026-08-09.md` ③. 뒷정리 경로 여섯이 **구현은 되어 있는데 한 번도 검증된 적이 없었다.**

### 여섯이 사실 하나였다

경로마다 따로 볼 생각이었는데, 파 보니 **전부 같은 두 걸음**으로 모인다.

```
공개 URL  →  toXxxStoragePath()  →  경로
경로      →  storage.remove()    →  지운다
```

그리고 그 계층 전체가 **한 가지 위험**을 안고 있다.

> `remove()`는 **없는 경로를 지워도 성공한다.**

경로를 한 글자라도 틀리면 **오류 없이 아무 일도 안 일어난다.** 파일은 쌓이고 신호는 없다.
그래서 검증의 질문이 "지웠나"가 아니라 **"우리가 만든 경로가 진짜 파일과 맞나"** 가 됐다.

단위 테스트가 되짚기를 이미 보고 있었지만, 그 테스트가 아는 공개 URL은 **사람이 손으로 적은
모양**이다. Supabase가 실제로 만드는 주소가 그 모양이 맞는지는 아무도 확인한 적이 없다.
그래서 **왕복**을 본다 — 진짜로 올리고 → 진짜 URL을 받고 → 되짚어 → 진짜로 지워지는지.

### 결과

| # | 경로 | 검증 |
|---|---|---|
| 1 | 프로필 사진 **교체** | ✅ `avatarCleanup.int.test.ts` |
| 2 | 프로필 사진 **삭제** | ✅ 〃 |
| 3 | 게시물 사진 **교체·삭제** | ✅ `postImageCleanup.int.test.ts` |
| 4 | 게시물 **삭제** | ✅ `postDeleteCleanup.int.test.ts` |
| 5 | 채팅방 **나가기** | 🔸 **부분** — 아래 참고 |
| 6 | 업로드 **중간 실패** 보상 | ✅ `postImageCleanup.int.test.ts` |

### 파다가 나온 것 셋

**① 스토리지가 한글 키를 거절한다.** `Invalid key`. 공백은 받고 공개 URL에서 `%20`이 된다.
그래서 `toPostImagePath`의 `decodeURIComponent`는 **쓸모가 있고**(공백), 한글로는 시험조차
할 수 없다. 앱은 `{stamp}-{index}.{ext}`로만 올리므로 실제로는 둘 다 안 생긴다 —
그래도 되짚기가 인코딩을 다룰 줄 아는지는 확인해 두었다.

**② 알림에는 cascade가 없다.** 0031이 "실제로 남는 것을 확인했다"고 **손으로 적어 둔 관찰**을
테스트로 굳혔다. 방도 메시지도 사라졌는데 알림만 남아 눌러도 갈 곳이 없다 —
`purge_chat_room`이 그것을 직접 지우는 이유다.

**③ 알림은 직접 심으면 안 됐다.** 처음에 손으로 하나 넣었더니 **둘**이 나왔다.
말이 오면 트리거가 이미 하나 만든다(0008). 손으로 넣은 것을 걷어내고 **트리거가 만든 것**을
본다 — 그편이 실제로 일어나는 일이기도 하다.

### 4번은 순서가 왜 그런지까지 확인했다

`deletePost`가 **지우기 전에 경로를 읽는** 이유를 두 가지로 나눠 봤다.

1. 글을 지우면 `post_images`·`chat_rooms` 행이 **따라 사라진다** → 그래서 미리 읽어야 한다
2. 행이 사라져도 **파일은 그대로 남는다** → 그래서 파일은 따로 지워야 한다

**2번이 이 계층 전체가 존재하는 이유다.** DB가 파일까지 치워 준다면 `deletePost`의 뒷부분도,
회원탈퇴의 훑기도, 이 테스트도 전부 필요 없다.

### 5번에 남은 벽

**`leave_chat_room`·`purge_chat_room`은 부를 수 없다.** 둘 다 `security definer`인데
`auth.uid()`가 참여자인지 직접 확인하고, 테스트는 세션을 못 만든다(Email 프로바이더를 껐다).
**서비스 키로 불러도 `auth.uid()`는 null**이라 `insufficient_privilege`로 막힌다.

그래서 재는 것을 **판정 뒤에 오는 일들**로 옮겼다 — 방 삭제가 메시지를 cascade하는가,
알림이 남는가, 파일이 남는가. 그 벽 자체도 테스트로 적었다(**남의 방은 못 지운다**는
확인이기도 하다).

**"양쪽이 다 나갔는가"라는 판정(`private.is_chat_room_purgeable`)은 화면에서 사람이 밟아야
한다.** 이것이 이 갈래의 정확한 경계다.
— ✅ **2026-08-13에 밟았다.** 맨 뒤의 "양쪽이 다 나간 방 — 화면에서 밟았다" 절.

### 확인

- `npm test` **894개** (기존 874 + 새 20)
- lint · tsc 통과 · 버킷 셋 다 비어 있음(테스트가 심은 것을 전부 치웠다)

---

## README — 저장소 첫 화면 (2026-08-09)

`plan-2026-08-09.md` ④. `git ls-files "*.md"`에 README가 없어서 GitHub 첫 화면이 파일
목록만 나왔다. 문서는 이미 9,800줄 넘게 쌓여 있으니 **입구만 만들면 됐다.**

### 스크린샷을 어떻게 찍었나

계획은 "스크린샷 두어 장"이라고만 적어 두었는데, **찍을 방법이 없었다.**
Playwright는 안 들이기로 했고(⑧C) 저장소에 브라우저 자동화가 없다.

그런데 **윈도우에 Edge와 Chrome이 이미 있다.** 헤드리스 스크린샷은 자동화 라이브러리 없이
CLI 한 줄로 된다.

```
chrome --headless=new --disable-gpu --hide-scrollbars \
       --user-data-dir=<매번 새로> --virtual-time-budget=12000 \
       --window-size=1280,900 --screenshot=out.png http://localhost:4390/posts/1
```

세 가지를 배웠다.

- **모바일 폭으로는 못 찍는다.** `--window-size=390,844`로 주면 **레이아웃은 800px로
  잡히고 왼쪽 390px만 잘려 나온다**(헤드리스의 최소 뷰포트). 그래서 데스크탑 폭으로 찍었다.
- **`--user-data-dir`를 매번 새로 줘야 한다.** 재사용하면 두 번째부터 조용히 실패한다
  (파일이 안 생기는 것이 유일한 신호다).
- **Edge가 옛 이미지를 돌려줬다.** 같은 자리를 Chrome으로 다시 찍으니 새 이미지가 나왔다.
  스크린샷은 **반드시 눈으로 확인**해야 한다 — 성공 종료코드가 최신을 보장하지 않는다.

### 빈 DB로는 찍을 것이 없다

계정 0 · 글 0이라 목록이 전부 비어 있다. 그래서 **잠깐 심고 찍고 지웠다.**
예시 글 넷과 판매자 하나를 서비스 키로 넣고, 상품 사진 자리에는 Pillow로 만든
그라데이션 색판을 넣었다(단색으로 처음 만들었더니 **버그처럼 보였다** — 글자를 얹으니
비로소 "예시 이미지"로 읽힌다).

치운 뒤 **직접 세어 확인했다** — 글 0 · 프로필 0 · 버킷 셋 다 0.
"지웠다"고 믿지 않고 세는 것은 통합 테스트에서 쓰던 태도 그대로다.

로그인 없이 열리는 화면만 찍을 수 있었다(`/posts/:id` · `/search` · `/login`).
홈·채팅·마이페이지는 세션이 필요해서 **여전히 사람이 찍어야 한다.**

### 무엇을 담았나

한 줄 소개 · 배포 주소 · 화면 셋 · 기능 요약 · 기술 스택 · 로컬 실행 · 테스트 · 문서 안내.

**테스트 절에 힘을 줬다.** 네 갈래가 각각 다른 질문을 한다는 것이 이 저장소에서 가장
설명할 값어치가 있는 구조라서다. `smoke`가 빨간데 `build`가 초록이면 **코드가 아니라
배포가 밀린 것**이라는 한 줄을 README에 넣어 두었다.

`SUPABASE_SERVICE_ROLE_KEY`에 `VITE_` 접두사를 붙이면 안 된다는 경고도 옮겨 적었다 —
처음 클론하는 사람이 가장 밟기 쉬운 지뢰다.

---

## 업로드 크기 상한 — 두 개로 나눴다 (2026-08-09)

`plan-2026-08-09.md` ⑦. "5MB를 10~12MB로 올릴까"라는 물음이었는데, **파 보니 그냥 올리면
안 되는 것이었다.**

### 버킷이 이미 5MB에서 막고 있었다

```
post-images 버킷   file_size_limit = 5242880   (0005)
클라이언트         MAX_POST_IMAGE_BYTES = 5MB
```

**두 곳이 같은 값으로 막고 있다.** 클라이언트 상수만 12MB로 올리면 사용자는 고를 수 있게
되지만 **서버가 거절한다.** 막는 자리가 앞에서 뒤로 옮겨질 뿐이고, 실패는 더 늦고 더
알아보기 어려워진다.
...
```
클라이언트 검사 →  고를 때, 원본에 걸린다
       ↓
   downscaleImages()     4MB → 483kB
       ↓
버킷 file_size_limit →  올릴 때, 줄인 뒤에 걸린다
```

**같은 5MB인데 서로 다른 것을 재고 있었다.** 그래서 고화소 폰 사진이 —
줄이면 500kB도 안 되는데 — **줄여 보지도 않고 거절**당했다.

### 상한을 둘로 나눴다

| 상수 | 값 | 무엇에 걸리나 |
|---|---|---|
| `MAX_IMAGE_SOURCE_BYTES` | **12MB** | 고를 때, **원본** |
| `MAX_POST_IMAGE_BYTES` | 5MB | 올릴 때, **줄인 뒤**(버킷과 같은 값) |

(원본 상한은 처음에 `MAX_POST_IMAGE_SOURCE_BYTES`라는 게시물 전용 상수였다.
다음 날 셋이 나눠 쓰게 되면서 `shared/utils/imageSizeLimit`으로 올라갔다 — 아래 후속.)

계획이 걱정한 자리는 이렇게 풀린다.

> **"축소가 실패하면 원본이 그대로 올라간다 — 상한을 올리면 그 최악의 경우도 함께 커진다."**

**안 커진다.** 저장 상한을 안 건드렸기 때문이다. 줄이기가 실패해 원본으로 물러나면
(`downscaleImage`는 어떤 실패에도 원본을 돌려준다) **업로드 직전에 다시 재서 세운다.**
그러니 저장되는 파일의 최악은 여전히 5MB다.

원본 상한을 12MB로 둔 이유는 용량이 아니라 **메모리**다. 디코드는 픽셀을 통째로 펼치므로
8000×6000이면 RGBA로 192MB다. 낮은 사양 기기에서 탭이 죽을 수 있다.

### 여기 걸리는 사진은 무엇인가

줄이기가 원본을 돌려주는 경우는 셋이다.

1. **GIF** — 캔버스에 그리면 움직임이 첫 장면으로 납작해져서 아예 손대지 않는다
2. 긴 변이 이미 1600px 이하 — 줄일 것이 없다(무손실 PNG가 여기 걸린다)
3. 디코드·인코드 실패, 또는 재인코딩이 오히려 커지는 경우

즉 새 검사에 걸리는 것은 **"줄일 수 없는데 큰" 사진**이고, 문구도 그렇게 적었다 —
"줄여도 5MB를 넘는 사진이 있습니다."

### 문구가 사용자에게 닿게 하는 데 한 걸음 더 필요했다

`newPostPage`·`postEditPage`가 `toPostErrorMessage`를 쓰는데, 그쪽은 **패턴에 안 걸린
한국어를 "잠시 후 다시 시도해 주세요"로 뭉갠다.** 되풀이해도 안 되는 일이라 거짓말이 된다.

`toPostActionErrorMessage`가 이미 그 자리를 위해 있었다(0010의 RPC 문구를 그대로 보여주려고
만든 것). 두 화면을 그쪽으로 옮겼다. `postStatusControl`이 같은 이유로 이미 그것을 쓰고 있고,
그 파일에 왜 그런지가 주석으로 적혀 있었다.

### 안 건드린 것 → 다음 날 건드렸다

**채팅 사진과 프로필 사진도 같은 모양이었다** — 셋 다 줄여서 올리는데 상한은 원본에 걸린다
(채팅 5MB · 아바타 2MB). 계획 ⑦이 게시물 사진만 짚어 그날은 거기까지 했고
`backlog.md` §1-5로 남겼다. 아래 후속에서 닫았다.

### 확인

- `npm test` **904개** (기존 894 + 새 10)
- lint · tsc 통과

**한 번 흔들렸다.** `npm test`를 네 번 돌려 세 번 통과, 한 번은 **한 스위트에서 17개**가
빨갰다(어느 스위트인지 못 잡았다 — 다시 돌리니 안 났다). 유닛과 통합을 따로 돌리면 둘 다
통과한다(817 + 87). 통합이 **실제 DB 하나를 공유**하므로 경합이 의심되지만 확증은 없다.
`troble.md`에 적어 두고, 다시 나면 그때 잡는다.

---

### 후속 (2026-08-10) — 채팅·프로필도 같은 모양으로 나눴다

`backlog.md` §1-5를 닫았다. **판단은 "게시물과 똑같이 한다"였다.** 셋 다 폰 갤러리에서
같은 사진을 고르고, 셋 다 `downscaleImage`로 줄여서 올린다. 다를 이유가 없다.

#### 1. 원본 상한은 셋이 하나를 쓰고, 저장 상한은 각자 갖는다

두 상한이 재는 것이 다르다는 것이 어제의 발견이었는데, **그 차이가 셋 다에서 같은 모양**이다.

| | 고를 때(원본) | 올릴 때(줄인 뒤) | 버킷 |
|---|---|---|---|
| 게시물 | 12MB | 5MB | `post-images` (0005) |
| 채팅 | 12MB | 5MB | `chat-images` (0008) |
| 프로필 | 12MB | 2MB | `avatars` (0002) |

원본 상한이 셋 다 같은 값인 이유는 **버킷과 무관하기 때문**이다 — 그것이 재는 것은 저장
용량이 아니라 **디코드가 감당할 픽셀**이고, 그건 어느 기능에서 고르든 같은 폰, 같은 탭이다.
그래서 `shared/utils/imageSizeLimit.ts`로 올렸다(`MAX_IMAGE_SOURCE_BYTES` ·
`findOversizedImage` · `toMegabyteText` · `validateSourceImageSize`).

**저장 상한은 올리지 않았다.** 버킷의 `file_size_limit`과 같은 값이어야 하는 값이라
클라이언트에서 혼자 올리면 서버가 거절할 뿐이다(어제 배운 것 그대로). 프로필이 2MB로
가장 작은 것도 그대로 뒀다 — 40px로 보이는 사진이라 더 클 이유가 없다.

#### 2. 상한을 나눴으면 **두 번째 그물도 같이** 놓아야 한다

여기가 이번에 조심한 자리다. 원본 상한만 올리고 끝내면 **GIF처럼 줄일 수 없는 큰 사진이
서버까지 갔다가 거절**당한다 — 사용자에게는 이유를 알 수 없는 실패다. 그래서 게시물의
`validateUploadableImages`와 짝이 되는 것을 둘 다 새로 놓았다.

```
validateUploadableChatImages   → chatApi.sendOneImageMessage   (downscale 직후)
validateUploadableAvatar       → profileApi.uploadAvatar        (downscale 직후)
```

재는 자리는 **줄인 직후, 업로드 직전** 한 곳뿐이다. 채팅은 한 장씩 올리므로(사진 한 장이
메시지 한 건) 한 장씩 재고, 그래서 실패해도 앞서 보낸 사진은 그대로 남는다 — 순차 전송의
기존 성질과 같다.

#### 3. 화면에 손으로 적은 MB가 이미 거짓말을 하고 있었다

`postImagePicker`의 안내 문구가 **"한 장에 5MB까지 올릴 수 있어요"** 인 채로 남아 있었다.
어제 상한을 12MB로 올렸는데 화면만 옛말을 하고 있었던 것이다. 아무도 못 알아챈 이유는
**틀려도 정상으로 보이기 때문**이다(0018의 "알 수 없는 이웃님이"와 같은 종류).

`toMegabyteText(MAX_IMAGE_SOURCE_BYTES)`로 바꿔 상수를 그대로 읽게 했다. `avatarPicker`의
"2MB 이하"도 같이 바꿨다. **안내 문구가 적는 것은 고를 때의 상한**이다 — 저장 상한은
줄인 뒤에 걸리는 값이라 고르는 사람이 알 수 없고, 알아도 할 수 있는 일이 없다.

#### 4. 서버 오류 문구는 안 고쳤다

`chatErrorMessage`의 "5MB 이하로 보내 주세요" · `profileErrorMessage`의 "2MB 이하로 올려
주세요"는 **버킷이 거절했을 때** 나오는 문구라 저장 상한이 맞다. 고를 때의 문구(12MB)와
숫자가 달라 보이지만 **다른 사건에 대한 다른 말**이다.

#### 확인

- `npm test` **921개**(유닛 834 + 통합 87) · lint · tsc 통과
- 새 테스트: `imageSizeLimit.test.ts`(8) · 채팅 상한 관계와 두 번째 그물 · 프로필 같은 것 ·
  `chatComposer`가 6MB를 그냥 보내는지
...
## 화면 확인용 시드 (2026-08-10)

`scripts/seedDemo.mjs`. **기능이 아니라 도구다.** 밀려 있던 화면 확인 아홉 건을 열려고 만들었다.

### 왜 필요했나 — 확인이 막힌 이유가 코드가 아니었다

배포도 끝났고 코드도 다 있는데 아홉 건이 밀려 있었다. 실제 DB를 열어 보고서야 이유를 알았다.

```
profiles 3 · posts 1 · chat_rooms 0 · messages 0 · comments 0 · reviews 0 · notifications 0
```

**절반 이상이 확인 자체가 불가능했다.** 채팅방이 없으면 메시지 삭제·나가기를 볼 수 없고,
후기가 없으면 매너온도 이력이 빈 화면이다. 알림 "모두 삭제"는 한술 더 뜬다 —
알림이 없으면 **버튼 자체가 안 뜬다**(그렇게 만들었다).

### 설계 결정 네 가지

#### 1. 로그인할 수 있는 계정은 못 만든다 — 그래서 "계정"이 아니라 "주변"을 심는다

로그인이 카카오·구글뿐이라 **심은 계정으로는 화면에 못 들어간다.** 관리자 API로 계정은
만들어지지만 그것으로 로그인할 길이 없다(Email 프로바이더는 껐다).

그래서 흔한 모양("데모 계정 만들어 그걸로 보세요")을 못 쓴다. 대신 **이미 로그인하는 계정을
받아 그 주변에 이웃과 사건을 심는다.** 보는 사람은 평소 쓰던 카카오 계정으로 그냥 들어간다.

동네도 그 계정에서 **읽어서 베낀다**(`region_code`·좌표). 손으로 적으면 한 글자만 달라도
심은 글이 그 사람 홈에 안 뜬다 — 심었는데 안 보이는 것이 가장 헷갈리는 실패다.

#### 2. 알림은 심지 않는다

댓글·찜·메시지·후기를 심으면 **트리거가 알아서 쌓는다**(0001 · 0015 · 0018).
직접 넣으면 트리거가 만드는 것과 payload 모양이 달라져, **화면이 진짜로 도는지를 못 본다.**
심고 나니 다섯 타입이 전부 찼다 — `comment 2 · like 8 · chat 5 · review 2 · price_offer 1`.
`notificationText.ts`의 갈래를 하나도 안 빼고 밟는다.

#### 3. 순서가 규칙이다 — 방이 먼저, 판매완료가 나중

거래완료 글을 만들려면 `buyer_id`를 박아야 하는데 `guard_post_buyer`(0035)가
**"거래 상대는 채팅한 사람 중에서만"** 을 지킨다. 방 없이 팔면 거절당한다.
0035가 정책에서 트리거로 옮긴 규칙이 여기서 그대로 걸린다 — **서비스 키로도 안 비껴간다**
(정책은 비껴가도 트리거는 돈다).

#### 4. 글마다 맡는 확인이 다르다

보는 사람 소유로 넷을 심는데 역할이 갈린다. 한 글에 다 얹으면 서로를 가린다 —
거래완료된 글에 **대기 중인 가격 제안**이 떠 있으면 그 화면이 거짓말을 한다.

```
0  댓글 · 비밀 댓글 실타래   (판매중)
1  채팅 · 대기 중인 가격 제안 (판매중)
2  거래완료 → 받은 후기
3  거래완료 → 받은 후기      (둘이라야 매너온도 이력에 줄이 두 개 생긴다)
```

### 치우는 규칙은 픽스처와 같다 — 심은 것만

`npm run seed -- --clean`. 이웃 계정을 지우면 그 사람의 글·댓글·채팅·후기가 cascade로
따라가고, **후기가 사라지면 매너온도도 되돌아간다**(0016). 거기까지가 "치웠다"의 뜻이다.
보는 사람 소유로 심은 글만 표(`[데모]`)로 찾아 지운다. 사람이 화면에서 만든 것은 안 건드린다.

### 확인

- 심은 결과: 글 12 · 채팅방 4 · 메시지 9 · 댓글 4(비밀 2) · 후기 2 · 알림 18 · 매너온도 **37.1**
- **등식이 맞는다**: 36.5 + 0.5 + 0.1 = 37.1. 0034가 이력을 남기는 이유가 이 확인이다.
- 시드를 넣은 채로 `npm test` **930개 통과** — 통합 테스트가 같은 DB를 쓰므로 이것부터 봤다.

## 알림에서 읽음을 없앴다 (2026-08-10)

`0036_notification_drop_read_state.sql`. **규칙이 하나로 줄었다.**

```
알림은 읽는 것이 아니라 치우는 것이다.

  줄을 누르면  → 가리키던 곳으로 가고 그 줄이 사라진다
  ×를 누르면   → 안 열고 그냥 사라진다
  모두 삭제    → 전부 사라진다
```

### 어떻게 여기까지 왔나 — 판단 셋이 이어 붙었다

한 번에 정한 것이 아니다. **시드를 심고 화면을 눈으로 보면서** 하나씩 밀렸다.

1. **"모두 읽음"이 애매하다.** 누르면 굵은 글씨가 보통 글씨가 될 뿐 **줄은 그대로 남는다.**
   알림 화면에서 하려는 일은 "치우기"인데, 옆의 "모두 삭제"와 나란히 서니 차이가 도드라졌다.
   → 없앴다.
2. **그러면 배지가 거짓말을 한다.** "모두 읽음"이 사라지면 배지가 0이 되는 길은 지우는 것
   하나뿐인데, 배지가 계속 *안 읽은 수*를 세면 **읽기만 하고 안 지운 알림**이
   `배지 0 · 목록 스무 줄`을 만든다. → 배지가 세는 것을 **남은 알림 수**로 바꿨다.
3. **그러면 읽음 상태가 아무 일도 안 한다.** 굵게 쓸지 말지에만 쓰이는데, 그 구분이
   배지와도 어긋나고 "치운다"는 규칙과도 어긋난다. → **누르면 지워지게** 하고 상태를 없앴다.

세 번째가 앞의 둘을 덮는다. **읽음을 없애면 1·2는 저절로 따라온다** — 없는 상태를 한꺼번에
바꿀 버튼도, 없는 상태를 셀 배지도 없다.

### DB에서 지운 것 — 칸 하나가 아니었다

```
notifications.is_read          컬럼
notifications_update           정책 (0001)
guard_notification_update      트리거 + 함수 (0015)
count_unread_notifications()   함수 (0015)
notifications_unread_idx       부분 인덱스 (0015)
fetch_notifications            반환 표에서 is_read 제거 (drop 후 create)
```

**가장 값이 나간 것은 정책이다.** 0015가 `guard_notification_update`를 붙여 지키려던 것은
"알림이 가리키는 대상이 사후에 바뀌지 않는다"였는데, **update 자체를 닫으면 더 확실하게
지켜진다.** 이제 알림에 허용되는 명령은 **select와 delete 둘뿐**이다(실제로 확인했다).

안 쓰는 칸을 남겨 두지 않은 이유도 같다. 아무도 안 부르는 update 경로가 열려 있으면
**그 위에 무엇을 얹어도 되는 줄 알게 된다.** 0017이 "정책만 남아 있는 것은 위험하지 않다"고
했다가 절반만 맞았던 자리다.

### 화면에서 갈린 것 하나 — 갈 곳 없는 알림

가리키던 방·글이 지워진 알림은 원래 링크가 아니라 그냥 줄로 그렸다(0019). 그 줄은
**눌러도 안 지워진다.** 갈 곳도 없는데 눌렀다고 사라지면 **무엇이 없어졌는지 확인할 길이
없기** 때문이다. 치우려면 ×를 쓴다.

`onSelect`를 `onOpen`으로 바꾼 것이 그 뜻이다 — "고른다"가 아니라 "연다"이고, 여는 순간
사라진다.

### 뮤테이션은 하나를 나눠 쓴다

줄을 눌러 열 때와 ×로 치울 때가 **하는 일이 같다**(그 줄이 사라진다). 다른 것은 누른 뒤
어디에 있느냐뿐이다.

캐시는 **응답을 받고 나서** 고친다. ×는 그 자리에 남으므로 실패했는데 줄이 사라졌다가
다시 나타나는 편보다 낫고, 눌러서 화면이 넘어간 경우에도 **뮤테이션에 걸어 둔 콜백은
컴포넌트가 사라져도 돈다** — 돌아왔을 때 그 줄은 이미 없다.

### 배지는 평범한 count로 센다

`count_unread_notifications()`는 `where is_read = false`가 박혀 있어 못 쓴다. 새 RPC를 만드는
대신 **평범한 count**로 옮겼다 — join도 payload 풀기도 필요 없고 **범위는 RLS가 정한다**
(삭제 쪽과 같은 약속). `head: true`라 행을 안 받고 숫자만 받는다.

그 요청이 **실제로 되는지는 코드를 읽어서 알 수 없어** 통합 테스트로 확인했다(익명이 세면
오류 없이 0). 조건 없는 delete를 확인한 것과 같은 자리다.

### 배포 순서에 걸리는 것

`main`이 아직 옛 코드를 서비스하는 동안 이 마이그레이션이 먼저 들어갔다. 그 사이 배포된
화면은 **배지가 0으로 보이고 모든 줄이 굵게** 나온다(없어진 RPC·칸을 부르기 때문이다).
**깨지지는 않는다** — 없는 정책은 오류가 아니라 "대상 행 없음"이고, 없는 RPC는 조회 실패라
배지가 0으로 떨어질 뿐이다. `develop`이 배포되면 사라지는 상태다.

### 확인

- `npm test` **925개** · lint · tsc 통과
- 마이그레이션 뒤 실제 DB에서 확인: `is_read` 칸 0 · 옛 RPC 0 · 부분 인덱스 0 ·
  트리거 0 · **정책은 SELECT·DELETE 둘뿐**
- 통합이 한 번 `JWT issued at future`로 빨갰다가 재실행에서 통과했다(`troble.md`의 그 자리).

## 고아 채팅 사진 청소 도구 (2026-08-10)

`scripts/sweepOrphanChatImages.mjs`. **탈퇴 검증을 하다가 나온 것이다.**

두 계정으로 사진을 주고받고 둘 다 탈퇴했더니 `chat-images`에 파일 하나가 남았다.
파 보니 원인은 코드가 아니라 **배포**였다 — 2026-08-09에 고친 버킷 훑기가
**Edge Function으로 배포되지 않은 채**였다(배포본은 v2, sweep 없음). 자세한 것은 `troble.md`.

### 배포는 v3로 밀었다. 그래도 도구가 필요했다

이미 생긴 고아 파일은 **앞으로 누구도 못 지운다.**

```
주인이 없다  → 그 사람의 탈퇴가 다시 일어날 일이 없다(sweep이 도는 유일한 순간)
방도 없다    → 정책이 "이 방 참여자인가"를 물을 근거가 없다
```

즉 **사람이 와서 치우는 수밖에 없는 종류**다. `backlog.md`에 "끊긴 뒷정리 다시 시도하기"로
적혀 있던 부채가 정확히 이 자리였다.

### 무엇을 고아로 보는가 — 규칙 둘

판정 기준은 하나다: **아무도 못 읽는 파일인가.** `chat_images_select`(0008)가 읽기를
허락하는 조건이 "그 방이 있고 내가 그 방 참여자일 것"이라, 그 조건이 **영영 참이 될 수 없으면**
쓰레기다. 그렇게 되는 길이 둘이다.

| | 규칙 | 걸리는 경우 |
|---|---|---|
| ① | 경로 둘째 칸(`user_id`)이 `auth.users`에 없다 | 참여자 자리가 비었다 |
| ② | 경로 첫 칸(`room_id`)이 `chat_rooms`에 없다 | **주인이 살아 있어도** 볼 근거가 없다 |

**②가 "상대가 올린 사진" 자리다**(2026-08-10에 더했다). A가 탈퇴하면 방이 cascade로 사라지는데
그 방에 B가 올린 사진은 B가 멀쩡히 살아 있어 ①에 안 걸린다. B가 탈퇴할 때 훑기가 줍기는 하지만
**B가 영영 탈퇴하지 않으면 영영 남는다.**

방 번호는 다시 쓰이지 않으므로(`generated always as identity`) ②는 **되돌아볼 여지가 없는
판정**이다. 지금 없는 방이 나중에 살아나지 않는다.

살아 있는 계정·방 목록을 **한 번에 받아 두고** 메모리에서 판단한다. 파일마다 물으면 왕복이
파일 수만큼 든다.

### 기본이 "찾기만"인 이유

되돌릴 수 없어서다. 목록을 눈으로 보고 `--go`를 붙인다 — 탈퇴 확인 문구(`탈퇴합니다`)와
같은 결의 장치다. 실제로 이번에도 먼저 찾아 한 줄을 눈으로 보고 지웠다.

### 확인 — 상황을 만들어 끝까지 밟았다

`scripts/`의 도구에는 이 저장소가 유닛 테스트를 붙이지 않는다(`seedDemo`·`runSmoke`도 그렇다).
대신 **실제 스토리지에 세 경우를 심어** 밟았다. 오검출이 없는지가 가장 중요하므로
"남아야 하는 파일"을 반드시 섞었다.

| 심은 것 | 기대 | 결과 |
|---|---|---|
| 죽은 방 + **살아 있는** 주인 | 지워짐(②) | ✅ `[방 없음]`으로 잡힘 |
| 죽은 방 + 없는 주인 | 지워짐 | ✅ 잡힘 |
| **살아 있는 방 + 살아 있는 주인** | **남음** | ✅ 목록에 안 뜸, `--go` 뒤에도 그대로 |

첫 줄이 규칙 ②를 더한 이유 그 자체다 — 전에는 이것이 안 잡혔다.
밟은 뒤 심은 것과 검증용으로 만든 방까지 되돌렸다(파일 먼저, 방 나중 — 0031이 정한 순서).

앞서 실제 탈퇴에서 나온 고아 하나도 이 도구로 치웠다. 지금 **세 버킷 전부 0개**다.

## 픽스처가 계정을 흘리던 자리 (2026-08-10)

`실제 DB에 붙는 통합 테스트`(2026-08-08)가 세운 규칙은 **"심은 것만 치운다"** 였다.
그 규칙이 반쯤만 지켜지고 있었다 — `auth.users`에 `int-test-*` 계정이 넷 쌓여 있었다.

### 등록이 한 줄 늦었다

`createTestUser`가 **계정을 만든 뒤 온보딩을 채우고 나서야** 픽스처에 등록했다.
그 사이에서 던지면 **계정은 있는데 픽스처는 모르는 상태**가 된다.

```
① createUser        계정이 생긴다
② profiles update   여기서 던지면
③ fixture.users.push  ← 여기에 못 온다 → cleanup이 지울 수 없다
```

②를 터뜨린 것은 `JWT issued at future`였다. **GoTrue(①)는 통과하고 PostgREST(②)만
거절당하니 정확히 이 틈이 벌어진다.** 남은 넷이 전부 "닉네임은 트리거 기본값,
`onboarded_at`은 null"이었던 것이 그 증거다.

**③을 ① 바로 뒤로 옮겼다.** 뒤에 무엇이 실패해도 치울 수 있다.

### cleanup이 실패를 삼키고 있었다

지우기 결과를 안 보고 목록을 비웠다. 못 지운 계정이 있어도 아무도 몰랐고, 나중에 흔적을
봐도 **언제 어디서 샜는지** 알 수 없었다. 이제 못 지운 것은 목록에 남기고 **예외를 던진다.**

**앱의 뒷정리와 반대 방향의 판단이다.** `delete-account`는 파일을 못 지워도 조용히 넘어간다 —
사용자가 할 수 있는 일이 없기 때문이다. 픽스처는 읽는 사람이 개발자이고 **지금 고칠 수 있다.**

### 확인 — 고친 자리가 바로 증명됐다

고친 뒤 돌린 `npm test`에서 **같은 흔들림이 또 났다**(10개 실패, 위의 시계 문제).
그런데 이번에는 `int-test-*` 계정이 **0개**였다 — 고치기 전이라면 하나가 남았을 자리다.
쌓여 있던 넷은 관리자 API로 지웠다.

## Edge Function 배포를 잡는 그물 (2026-08-10)

`src/smoke/deployedFunction.smoke.test.ts`. **오늘 뚫린 자리를 막는다.**

### 무엇이 없었나

그물이 넷인데 **전부 프런트엔드만 본다.**

| 갈래 | 무엇을 묻나 |
|---|---|
| `unit`·`integration` | 로직이 맞는가 |
| `build` | 제대로 만들어졌는가(`dist/`) |
| `smoke` | 제대로 서비스되는가(배포 주소) |

프런트엔드는 Vercel이 `main`을 보고 **자동으로** 나가므로 "머지했는데 안 나갔다"가 생기기
어렵다. **함수는 아무도 안 밀어 준다.** 그 틈으로 2026-08-09의 수정이 하루를 그냥 지났고,
그동안 **테스트는 내내 초록이었다.**
...
CORS_HEADERS['x-function-revision'] = FUNCTION_REVISION;
```

스모크가 그 값을 저장소의 소스에서 읽은 값과 견준다. 어긋나면 **"배포가 안 됐다"**로 빨개진다.

#### 왜 하필 OPTIONS인가 — 키 없이 물어볼 수 있는 유일한 창

`verify_jwt: true`라 GET·POST는 게이트웨이가 401로 막는다. 실제로 재 봤다.

```
OPTIONS → 204  + 함수가 붙인 헤더가 그대로 온다
GET     → 401
POST    → 401  {"code":"UNAUTHORIZED_NO_AUTH_HEADER"}
```

**preflight만 검사 없이 함수까지 간다.** 그래서 판을 CORS 헤더에 실었다 — 브라우저가 읽을
일은 없고, **밖에서 물어보는 것이 이 헤더의 목적**이다.

주소는 이 갈래의 규칙대로 **배포된 번들에서 뽑는다**(`findSupabaseUrl`). 실제로 서비스되는
값으로 검사한다는 뜻이다.

### 그물이 진짜 잡는지 확인했다

**초록만 보고 끝내지 않았다.** 저장소의 판만 바꿔(배포본은 그대로) 돌려 보니 정확히 빨개졌다.

```
● 배포된 판이 저장소와 같다
    Expected: "9999-99-99.일부러-어긋냄"
    Received: "2026-08-10.chat-image-sweep"
```

이 저장소가 "건너뛰는 스모크는 초록인 채로 아무것도 안 지킨다"고 적어 둔 것과 같은 이유다 —
**빨개지는 것을 본 적 없는 그물은 그물이 아니다.**

### 한계를 적어 둔다

**판은 손으로 올린다.** 함수를 고치면서 판을 안 올리면 그물이 못 잡는다. 자동으로 만들려면
소스 해시를 심어야 하는데 빌드 단계가 따라온다.

그래도 **"아무 그물도 없음"보다 낫다** — 오늘 놓친 것은 판이 어긋난 것이 아니라
**아무도 안 본 것**이었다. 상수를 고치는 자리 바로 위에 두어 눈에 걸리게 했다.

### 확인

- `npm run test:smoke` **23개**(기존 21 + 새 2) · `npm test` 925개 · lint · tsc
- 함수를 v4로 배포하고 헤더가 실제로 오는 것을 확인했다.

## 댓글 페이징 — 1단 열 개씩 (2026-08-10)

`backlog.md`가 "댓글이 수십 개 달리기 전에는 볼 것이 없다"고 미뤄 두었던 것을 **사용자가
열 개로 정하면서** 열었다. 마이그레이션은 없다 — 0017의 인덱스가 `(post_id, created_at)`이라
그대로 받는다.

### 자를 곳은 실타래 사이여야 한다

**평평하게 열 줄씩 끊으면 안 된다.** 답글이 부모와 갈라져 다음 페이지로 넘어가면
`buildCommentTree`가 **부모 없는 답글을 1단으로 올려** 그린다 — 그쪽은 차단으로 부모가
사라진 경우를 위해 일부러 그렇게 만든 자리다(맥락 없는 한 줄이라도 남기는 편이 낫다).
페이징이 그 길을 밟으면 **답이 물음처럼 보인다.**

그래서 **1단만 세어 끊고, 그 1단들의 답글은 함께 가져온다.**

```
① 1단 열 개    post_id + parent_id is null + (created_at, id) keyset
② 그 답글들    parent_id in (①의 id들)
```

한 페이지에 오는 줄 수는 열 개보다 많을 수 있다 — **답글은 안 세어진다.**

### 왕복 둘을 RPC로 합치지 않았다

②는 ①의 결과를 알아야 물을 수 있어 **어차피 순서가 있다.** 정책만으로 되는 일에 서버 함수를
만들지 않는다 — 차단·비밀 댓글을 거르는 일은 양쪽 질의에서 `comments_select`가 그대로 한다
(0017 · 0033). 함수로 옮기면 그 규칙을 다시 적어야 한다.

### 미뤄 둔 이유 하나는 이미 사라져 있었다

옛 주석은 페이징을 미룬 이유를 둘로 적었다. 하나는 "쌓이지 않는다"이고, 다른 하나가
**"나누면 댓글 개수를 따로 세어 와야 한다"** 였다. 뒤엣것은 **0028이 `posts.comment_count`를
만들면서 이미 풀려 있었다.**

머리말은 그 값을 쓴다. 받아 온 줄을 세면 "댓글 10" 아래에 더 보기 버튼이 남는 꼴이 된다.
상세 조회에 `comment_count`가 안 실려 있어 한 칸 더했다(카드에는 이미 있었다).

**비밀 댓글은 안 세어진다**(0033). 볼 수 있는 사람에게는 화면의 줄 수보다 작을 수 있는데,
그것이 0033이 고른 기준이다 — 평범한 컬럼이라 RLS가 안 걸려 **보는 사람마다 다를 수 없고,
그러면 기준은 가장 적게 보는 사람**이어야 한다. 카드의 "댓글 n"과 같은 숫자다.

### 무한 스크롤이 아니라 버튼이다

이 목록은 **상세 화면 안**에 있다. 스크롤로 이어 붙이면 페이지 끝에 닿을 때마다 댓글이 자라
**그 아래에 있는 것(입력칸·다음 화면)에 영영 못 닿는다.** 홈·검색처럼 목록이 화면의 전부인
자리와 다르다.

### 캐시는 알림과 같은 모양으로

세 뮤테이션(쓰기·고치기·지우기)이 `InfiniteData`를 만지게 되면서 `commentCache.ts`로 모았다
(`notificationCache`와 같은 자리·같은 이유). **0017이 겪은 것을 그대로 지킨다** —
부모를 지울 때 답글도 함께 빼야 남은 답글이 1단으로 떠오르지 않는다. 그 규칙을 테스트가
트리까지 그려 확인한다.

### 확인

- `npm test` **938개**(기존 925 + 새 13) · lint · tsc
- **keyset 문법은 실제 서버에 물어봤다.** `.or('created_at.gt.…,and(created_at.eq.…,id.gt.…)')`가
  PostgREST에서 도는지는 코드로 알 수 없어, 실제 DB에 1단 셋 + 답글 하나를 심고 커서 뒤를
  물었다 — **1단 둘만 오고 답글은 안 섞였다.** 심은 것은 지웠다.

## 댓글 알림을 글마다 한 줄로 (2026-08-10)

`0037_comment_notification_grouping.sql`. 0018은 댓글 하나에 알림 한 줄을 넣었다.
한 글에 댓글이 열 개 달리면 **알림도 열 줄**이고, 그 열 줄이 전부 같은 곳을 가리킨다 —
눌러도 같은 글, 지워도 같은 글이다. **받는 사람 × 글마다 한 줄**로 모았다.
...
- **update가 된다.** 0036이 알림의 update를 통째로 닫았지만 **그것은 사용자 경로**다.
  넣는 트리거는 `security definer`라 소유자 권한으로 돌고 RLS를 지나간다 —
  0023이 매너온도에서 "시스템은 그대로 통과한다"고 가른 자리와 같다.
- **묶음을 되돌릴 자리가 필요 없다.** "누르면 지워진다"(0036)라서 확인한 줄은 그 순간
  사라지고 다음 댓글은 개수 1부터 새로 시작한다. 읽음 상태가 남아 있었다면
  **"읽은 뒤에 온 댓글은 새 줄인가 같은 줄인가"** 를 정해야 했다.

### 실제 DB에 물어보다 버그를 잡았다

처음 판은 `update … where user_id = … and post_id = …` 뒤에 `row_count`를 보고 갈래를
타는 모양이었다. **대상을 하나로 좁히지 않았다.** 데모로 심어 둔 댓글 알림 열네 줄이
이미 있었는데, 새 댓글 하나에 **그 열네 줄이 전부 같은 값으로 덮였다.**

```
post_id 40802 · comment_count 4 · 최신 comment_id 734   ← 이 줄이 열네 번 반복
```

묶인 것이 아니라 **복제됐다.** 테스트로는 안 나왔을 자리다 — 중복이 이미 있는 상태를
만들어 두지 않으면 재현되지 않는다.

**고친 방향은 함수가 아니라 스키마다.**

```sql
create unique index notifications_comment_per_post_idx
  on notifications (user_id, ((payload ->> 'post_id')::bigint))
  where type = 'comment';
```

규칙을 부분 유일 인덱스로 못 박으면 ① 함수가 아무리 잘못 짜여도 두 줄이 될 수 없고
② `on conflict`를 쓸 수 있어 **넣기와 새로 고치기가 한 문장**이 된다(경합에도 안전하다).
0035가 "한 규칙이 두 명령에 걸려야 하면 정책은 잘못된 도구다"라고 적은 것과 같은 결이다 —
**불변식은 지키는 쪽에 두어야 한다.**

이미 쌓여 있던 중복은 마이그레이션이 접는다. 가장 최근 것 하나만 남기고 개수를
접은 줄 수로 채운다 — 규칙만 바꾸고 두면 옛 줄은 영영 안 접힌다.
...
**묶였을 때 사람 이름을 안 쓴다.** 아는 것이 최신 한 사람뿐이라 "외 2명"이라고 적으면
**같은 사람이 세 번 단 경우에 거짓말**이 된다(0018이 "알 수 없는 이웃님이"로 겪은 것과
같은 종류 — 틀려도 정상으로 보인다). 아바타는 최신 사람 것이 그대로 보인다.

### 안 묶는 것

**찜은 안 묶는다.** 같은 글에 여러 사람이 찜해도 한 사람이 여러 번은 못 한다(0001의 unique).
사람마다 한 줄이 오히려 맞고, 묶으면 누가 찜했는지가 사라진다.
채팅·가격 제안도 그대로다 — 그쪽은 **방이 이미 묶음 노릇**을 한다.

### 확인

- `npm test` **940개** · lint · tsc
- 실제 DB: 같은 글 댓글 3개 → **알림 1줄, 개수 3**, 최신 comment_id.
  손으로 같은 (사람, 글) 줄을 하나 더 넣어 보니 **23505로 막힌다.**

## 카카오톡 공유 (2026-08-10)

게시물 상세 오른쪽 위의 공유 버튼. 누르면 카카오톡 대화 상대를 고르고 **그 글의 카드**가 간다.

### SSR 없이 글마다 다른 카드가 된다

`backlog.md` §4는 **게시물별 공유 카드를 안 만들기로** 했다. 이유가 이랬다.

> 수집기가 HTML을 긁어갈 때 이미 값이 박혀 있어야 하는데 SPA는 서버가 HTML을 만들지 않는다.
> **카카오톡·슬랙 봇은 JS를 실행하지 않는다.**

**그것은 주소만 붙여 넣는 경우의 이야기다.** 카카오톡 공유 SDK는 다르다 —
`Share.sendDefault`가 **내용을 직접 실어 보낸다.** 카카오가 우리 페이지를 긁지 않으므로
사진·제목·가격을 그대로 담을 수 있다.

**그 판단을 뒤집은 것이 아니다.** 주소를 그냥 붙여 넣으면 지금도 사이트 공통 카드가 뜬다
(`index.html`의 og 태그, 0809). 이 길이 그 제약을 비껴갈 뿐이다.

### 지도 SDK를 나눠 쓸 수 없다

이름이 같은 카카오지만 **받아 오는 파일이 다르다.**

```
지도   dapi.kakao.com/v2/maps/sdk.js    → window.kakao.maps
공유   t1.kakaocdn.net/kakao_js_sdk/…   → window.Kakao.Share  (+ Kakao.init 필요)
```

소문자 `kakao`와 대문자 `Kakao`가 각각 다른 객체다. 그래서 로더를 하나 더 만들었다 —
다만 **앱키는 같은 것**을 쓴다(둘 다 JavaScript 앱키다).

**새 환경변수를 만들지 않은 이유가 있다.** 2026-08-09에 Vercel에 앱키를 넣다 **한 글자가
잘려** 지도가 통째로 죽었다. 손으로 옮기는 값은 적을수록 좋고, 이미 있는 값은
`test:build`가 형식과 길이를 지키고 있다. 판(SDK 버전)은 고정했다 — 받는 파일이 조용히
바뀌는 것은 배포와 무관하게 화면이 달라지는 길이다.

### 로그인을 묻지 않는다

공유는 **우리 서버에 아무것도 쓰지 않는다.** 누구에게 보낼지는 카카오톡에서 고르므로
우리가 아는 사람일 필요가 없다. 글만 보러 온 사람이 친구에게 넘기는 것이 오히려 흔한 경로다.

그래서 버튼을 ⋯ 메뉴 **밖**에 뒀다. ⋯는 내 글이냐 남의 글이냐로 갈리는데
공유는 양쪽에서 같고 비로그인에서도 같다.

### 안 되는 자리에 물러설 데를 둔다

SDK를 못 받거나(네트워크·도메인 미등록) 카카오톡이 없는 기기가 있다.
**아무 일도 안 일어나면 "눌렀는데 왜 안 되지"** 가 된다. 그래서 주소를 클립보드에 복사하고
그렇게 말해 준다 — **공유의 목적(이 글을 남에게 보낸다)은 그것으로도 이뤄진다.**
복사까지 막히면(권한 거부·비보안 문맥) 마지막으로 실패를 말한다. 조용히 삼키지 않는다.

### 잘린 자리를 우리가 정한다

카드 설명은 **가격이 먼저**고(중고거래에서 먼저 보는 값이다) 동네와 설명이 따라온다.
60자에서 자르는데, 카카오가 자르기 전에 우리가 자르는 편이 낫다 — 어디서 잘리는지를
우리가 정하게 된다. 처음 판은 **말줄임표 한 글자를 안 세어 딱 한 글자를 넘겼다**(테스트가 잡았다).

### 확인

- `npm test` **952개**(기존 940 + 새 12) · `test:build` 35개 · lint · tsc
- **콘솔 설정은 이미 돼 있다** — 카카오 개발자 콘솔의 Web 플랫폼 도메인 등록이 로그인·지도
  때문에 이미 끝나 있고, 공유도 같은 등록을 쓴다. 스모크가 배포 도메인으로 SDK를 받는 것을
  이미 지키고 있다.
- **실제로 보내 봤다** (2026-08-13, **배포 주소**). 카카오톡 대화 상대를 고르는 창이 뜨고
  그 글의 카드가 갔다. **테스트가 대신할 수 없던 마지막 한 칸**이다 — 앞의 두 줄은
  "우리가 만든 값이 맞나"까지만 말하고, **카카오가 그 값을 카드로 그려 주는가**는
  카카오톡이 깔린 기기에서만 알 수 있다.
  - 로컬이 아니라 **배포 도메인에서** 밟은 것이 중요하다. SDK를 받는 것도 카드가 가는 것도
    **콘솔에 등록된 도메인**이 근거라, localhost에서 되는 것은 배포에서 된다는 뜻이 아니다.

## 양쪽이 다 나간 방 — 화면에서 밟았다 (2026-08-13)

`0031`이 만든 길의 **마지막 한 칸**이다. 2026-08-06에 서버 규칙 아홉을 SQL로 밟았고,
2026-08-09에 스토리지 뒷정리 여섯 경로 중 다섯을 테스트로 덮었는데, **여섯 번째만 남았다.**

> **"양쪽이 다 나갔는가"라는 판정(`private.is_chat_room_purgeable`)은 화면에서 사람이 밟아야
> 한다.** — 2026-08-09에 적어 둔 경계

### 왜 테스트가 못 밟나 (다시)

`leave_chat_room`·`purge_chat_room`은 `security definer`로 `auth.uid()`를 직접 본다.
통합 테스트는 **서비스 키로 불러도 `auth.uid()`가 null**이라 `insufficient_privilege`로
막힌다. 로그인이 카카오·구글뿐이라 테스트가 세션을 만들 방법도 없다.

그래서 자동화가 재던 것은 **판정 뒤에 오는 일들**(cascade · 알림 잔존 · 파일 잔존)뿐이었다.
**판정 자체와, 화면이 세 걸음을 순서대로 부르는가**는 사람만 볼 수 있었다.

### 밟은 것

카카오 계정(판매자)과 구글 계정(구매자), 배포 주소에서. 재는 것은 SQL로 나눠 맡았다 —
**밟는 것은 사람, 세는 것은 DB**다.

| 단계 | 화면에서 한 일 | DB가 말한 것 |
|---|---|---|
| ① | 구매자가 말 + **사진**, 판매자가 답 | 방 1 · 메시지 3 · `chat-images` 1 · 알림 양쪽 · 판정 **false** |
| ② | 구매자만 나가기 | `buyer_left_at`만 찍힘 · `seller_left_at` **null** · 방 살아 있음 · 판정 **false** |
| ③ | 판매자도 나가기 | 방 **0** · 메시지 **0** · 사진 **0** · 알림 **0** · 글·계정·글 사진은 그대로 |

②가 ③만큼 중요하다. **한 명만 나갔을 때 지워지면 상대 화면에서 대화가 사라진다** —
0030이 나가기를 "목록에서만 감추는 일"로 정한 이유가 거기 있고, 화면에서도 상대 목록에
방이 그대로 있었다.

### 여기서만 보이는 것 셋

- **순서가 진짜로 지켜졌다.** 사진이 0이 됐다는 것은 **방을 지우기 전에** 폴더를 비웠다는
  뜻이다. 반대로 했으면 `chat_images_select`가 막혀 목록조차 못 읽고 파일이 남는다.
  코드에 적힌 순서와 도는 순서가 같은지는 **한 번은 눈으로 봐야** 알 수 있는 종류다.
- **알림 둘이 함께 사라졌다.** payload가 jsonb라 FK가 없어 `purge_chat_room`이 손으로
  걷어내는 자리다. **그 코드가 실제 사용자 경로에서 돈 것은 이번이 처음**이다.
- **판정의 아슬아슬한 쪽도 봤다.** 구매자가 나가기 직전에 말을 한 줄 더 보내
  마지막 말이 나간 시각보다 **4초 앞섰다.** `last_message_at <= least(...)`가 등호를 포함한
  쪽으로 맞았다 — 한 줄 더 늦게 왔으면 방이 남는 것이 정상이다(0030의 "돌아오는 방").

### 덤으로 확인된 것 — 알림은 누르면 지워진다

세는 동안 알림 id가 **1290 → (1291 없음) → 1292 → (없음) → 1293**으로 뛰었다.
받은 사람이 눌러 들어간 줄이 그때 사라진 자국이다(`0036`). 화면 밖에서 **id의 구멍으로**
보이는 것이 재미있는 자리다 — 지워진 행은 세어지지 않지만 번호는 거짓말을 못 한다.

## 알림은 **도착하면** 지워진다 (2026-08-13)

`0036`이 "알림은 누르면 지워진다"로 정한 뒤에 남아 있던 구멍이다. 마이그레이션은 없다.

### 증상

알림이 와 있는데 **알림 목록을 거치지 않고** 그 일을 확인하면 알림이 그대로 남는다.
채팅 목록에서 방을 열거나 홈에서 글을 눌러 들어간 경우다. 사람이 한 일은 같은데
**어느 문으로 들어왔느냐로 갈렸고**, 종 배지는 안 줄어든다.

### 무엇이 규칙이었나

0036이 읽음 상태를 없애며 정한 것은 "누르면 지워진다"였지만, 그 규칙이 하려던 말은
**확인한 알림은 남지 않는다**였다. 누르는 것은 확인하는 여러 방법 중 하나일 뿐이다.

그래서 기준을 **"눌렀는가"에서 "도착했는가"로** 옮겼다. 도착지는 새로 정하지 않았다 —
`notificationText`가 이미 알림마다 갈 곳(`view.to`)을 정하고 있어서 그것을 그대로 뒤집었다.

| 알림 | 데려가는 곳 | 도착하면 |
|---|---|---|
| `chat` · `price_offer` | `/chat/:roomId` | 그 방의 것이 사라진다 |
| `comment` · `like` | `/posts/:postId` | 그 글의 것이 사라진다 |
| `review` | `/users/:id` | **안 지운다** — 아래 |

**후기만 뺐다.** 받은 후기가 프로필의 **탭 뒤에** 있어서, 그 화면에 도착한 것이 그 후기를
본 것이 아니다. 도착과 봄이 갈리는 자리에 같은 규칙을 적용하면 **안 본 것을 지운다.**

### payload로 찾는다

알림에는 `room_id`·`post_id` 컬럼이 없다. jsonb 한 칸뿐이라(0015) 화면이 가진 id로
되짚는 길은 `payload->>'room_id'`뿐이다. PostgREST가 그 문법을 받는지는 **실제 서버에
물어 확인했다**(`payload->>room_id=eq.690` → 11줄).

**`user_id`로 좁히지 않는다.** 삭제 쪽의 오랜 약속이다(`deleteNotification`의 주석) —
범위를 정하는 것은 `notifications_delete` 정책 하나다. 그래서 남의 방에 도착해도 지워지는
것은 내 알림뿐이다. 이것도 실제 DB에서 밟았다: A로 세션을 흉내 내 방 690의 알림을 지우니
**4줄이 지워지고 주인은 하나**였다(그 방의 알림은 모두 11줄이다. 나머지는 상대 것이라 남는다).

### 곁가지의 규칙을 그대로 따랐다

`useMarkRoomRead`와 같은 모양이다. 화면이 열릴 때 한 번 돌고, **실패는 조용히 넘긴다** —
사용자가 누른 일이 아니고 다음에 그 화면을 열면 다시 시도된다. 방과 글은 이미 열려 있다.

**같은 곳에 두 번 도착한 것으로 치지 않는다.** StrictMode가 effect를 두 번 돌리는 데다,
부르는 쪽이 `{ kind: 'room', roomId }`를 그 자리에서 만들어 **렌더마다 새 객체**다.
객체를 의존성에 두면 방을 보고 있는 동안 삭제 요청이 꼬리를 문다 — effect는 **열쇠 문자열**을
보고, 값은 ref로 넘긴다.

### 배지는 지운 만큼만 깎는다

`withCountReducedBy`. "모두 삭제"처럼 0으로 놓으면 안 된다 — **방 하나에 도착했다고 다른
방의 알림까지 확인한 것은 아니다.** 몇 줄이 사라졌는지는 서버가 돌려준 id로 안다
(`delete ... select('id')`). 그래서 다시 세러 가는 왕복이 없다.

### 확인

- `npm test` **968개**(기존 952 + 새 16) · lint · tsc
- 실제 서버: PostgREST의 jsonb 필터 · RLS 아래의 삭제 범위(위)

## 네 계정이 얽힌 시드 (2026-08-13)

`scripts/seedStory.mjs` · `npm run seed:story`. 2026-08-10의 `seedDemo.mjs`와 **다른 도구**다.

### 왜 하나가 더 필요했나

그쪽은 **로그인하는 계정이 하나뿐**일 때를 위한 것이다. 이웃을 가짜로 만들어 그 주변에
사건을 심는데, 가짜 계정으로는 화면에 못 들어가니 **한쪽에서 본 모습**만 확인된다.

로그인할 수 있는 계정이 넷이 되자 그것으로는 부족해졌다. 비밀 댓글은 **보는 사람과 못 보는
사람**이 갈리고, 채팅은 양쪽 화면이 다르고, 후기는 준 쪽과 받은 쪽이 다르다 —
**네 계정으로 같은 자리를 열어 봐야** 확인되는 것들이다.

### 사진은 진짜를 받아 온다

물건 사진이 색 사각형이면 목록·격자·상세가 실제로 어떻게 보이는지 알 수 없다.
loremflickr(Flickr의 CC 사진)에서 물건 이름으로 받고, 프로필은 pravatar에서 받는다.
주소에 `lock`을 박아 **다시 심어도 같은 사진**이 오게 했다 — 화면을 다시 볼 때 물건이
바뀌면 무엇이 달라졌는지 헷갈린다.

### 표를 못 붙여서 목록을 적는다

`seedDemo`는 제목에 `[데모]`를 붙여 찾는다. 여기서는 **제목이 진짜 같아야 해서** 그 손을
못 쓴다. 대신 심은 id와 파일 경로를 `scripts/.seedStory.json`에 적고 `--clean`이 그것만
지운다. 사람이 화면에서 만든 것은 목록에 없으니 건드리지 않는다.
목록은 **실패해도 쓴다**(`finally`) — 반쯤 심다 죽었을 때 치울 방법이 없으면 곤란하다.

### 밟다가 고친 것 — 남의 사진을 덮었다

처음 판이 프로필 사진을 **무조건 덮어썼다.** 계정 넷 중 둘이 이미 사진을 올려 둔 상태였고,
덮는 순간 그 파일들이 **아무도 안 가리키는 파일**이 됐다(실제로 두 장을 그렇게 만들었다).
지우면 사람이 만든 것을 지우는 셈이고 두면 쓰레기다 — **아예 안 덮는 것**이 답이었다.
`avatar_url`이 이미 있으면 건너뛴다. 만들어 둔 고아 둘은 원래 사진을 되돌려 주며 치웠다.

### 심고 나서 확인한 것

심은 김에 **비밀 댓글이 정말 갈리는지**를 RLS를 실제로 적용해 세어 봤다.
같은 글(댓글 6개)을 세 사람이 보면 이렇다.
...
카드의 "댓글 n"도 공개만 센다(0033이 정한 대로). **네 계정이 없으면 못 보는 확인**이다.

### 두 번째 판 (`--more`, 2026-08-13)

첫 판을 그대로 다시 돌리면 **같은 글이 두 벌** 생긴다. 그래서 더 심는 것을 따로 적고
`--more`로 부른다. 글 11개·댓글 20개·채팅방 5개·후기 3개가 더 붙는다.

- **첫 판의 글에도 말이 붙는다.** 글 하나에 시간을 두고 댓글·찜이 쌓이는 모습은 그래야 나온다.
  첫 판의 글은 **제목으로 찾아** 붙이고(`findPlantedPosts`), **못 찾으면 그 줄만 건너뛴다** —
  사람이 지운 글일 수 있고 그때 시드가 멈출 이유가 없다.
- **목록은 이어 쓴다.** 새로 쓰면 첫 판이 심은 것을 치울 방법이 없어진다(`loadManifest`).
- **프로필 사진은 넷 다 건너뛴다.** 이미 있는 것을 안 덮는 규칙이 여기서 값을 했다.
- 첫 판에 없던 상태를 채웠다 — **거절된 가격 제안**과 **나쁜 후기(−0.5)**.
  이제 제안 네 상태(수락·대기·취소·거절)가 다 있고, 매너온도가 **내려가는 줄**도 이력에 있다.

심은 뒤 센 것: 글 24(판매중 19 · 예약중 1 · 거래완료 4) · 댓글 35(비밀 8) · 채팅방 10 ·
메시지 48 · 후기 6 · 찜 28 · 알림 105 · 파일 39.

---

## 홈을 비로그인에게 연다 (2026-08-20)

배포된 주소를 **처음 누른 사람이 아무것도 볼 수 없던** 자리를 고쳤다.
채용 담당자가 링크를 열면 로고 · 검색창 · "시작하기" 버튼만 있는 화면을 보고 되돌아갔다.

### 무엇을 만들었나

1. **기본 동네** (`region/utils/defaultGuestRegion.ts`) — 아직 아무것도 고르지 않은 사람에게
   대신 세우는 동네(석관동). 저장하지 않고 읽을 때만 선다.
2. **`useActiveRegion`에 `isDefaultRegion`** — 지금 동네가 사용자가 고른 것인지 대신 세운 것인지.
3. **`GuestRegionSwitcher`** (`region/components/`) — 게스트가 그 자리에서 동네를 바꾸는 위젯.
   홈 · 검색 · 지도 셋이 함께 쓴다.
4. **홈이 로그인 여부를 묻지 않고 목록을 그린다** — `GuestActions`는 목록을 대신하던 자리에서
   목록 위 배너(`GuestBanner`)로 내려왔다.
5. **`SearchRegionPrompt`에서 게스트 분기를 걷어냈다** — 닿지 않는 길이 됐다(아래 3번).

### 설계 결정 다섯 가지

#### 1. 새로 만든 것이 거의 없다 — 서버는 처음부터 열려 있었다

고치기 전에 **배포된 DB에 anon 키로 직접 물어봤다.** 로그인 없이:

| 물어본 것 | 답 |
|---|---|
| `posts` · `profiles` · `comments` · `reviews` · `post_images` · `categories` | 200, 실제 데이터 |
| `chat_rooms` · `messages` · `notifications` · `reports` | 빈 배열 |
| `search_posts`(석관동) | 글 7개, 썸네일까지 |
| `search_posts`(3km 반경) | 24개 전부 |

`posts_select`가 `using (true)`라 **익명도 지나가야 하는 자리**였고, `postSearch.int.test.ts`가
이미 그것을 **로그인하지 않은 채로** 밟고 있었다(테스트 클라이언트가 anon 키 · `persistSession: false`).
라우터도 `/search` · `/search/map` · `/posts/:id` · `/users/:id`가 이미 공개였고,
상세 화면은 "로그인하고 채팅하기"까지 갖고 있었다.

**닫혀 있던 것은 화면 한 줄뿐이었다** — `homePage.tsx`가 목록을 `isMember`로 감싸고 있었다.
그래서 이번 일은 새 기능이 아니라 **이미 있는 길의 첫 칸을 여는 일**이 됐다.
마이그레이션 0개 · 대시보드 설정 변경 0개다.

#### 2. 기본 동네를 저장하지 않는다

게스트의 동네는 localStorage에 남는데(`storedRegion.ts`), 기본값을 거기 써 버리면
**"아직 안 고른 사람"과 "석관동을 직접 고른 사람"이 같아진다.** 그러면 다음에 왔을 때
"여기가 당신 동네라고 정한 적 없다"를 말할 수 없다.

그래서 `useActiveRegion`이 **읽을 때만** 세우고(`guestRegion ?? DEFAULT_GUEST_REGION`),
고른 적이 있는지는 `isDefaultRegion`으로 따로 알린다.
`region`이 null인지로는 구분할 수 없다 — 기본값을 세우는 순간 언제나 채워져 있다.

**로그인 사용자에게는 세우지 않는다.** 그쪽에서 동네가 비어 있는 것은 온보딩이 맡는 일이라
(`RequireOnboarding`) 여기서 대신 채우면 그 화면이 영영 안 뜬다.

#### 3. 기본 동네와 `GuestRegionSwitcher`는 한 묶음이다 — 따로 넣을 수 없었다

게스트가 동네를 고르는 **유일한 입구**가 `SearchRegionPrompt`였고, 그것이 뜨는 조건이
`activeRegion.region === null`이었다. 기본 동네를 세우면 그 조건이 **영영 거짓**이 된다 —
즉 기본값만 넣으면 **게스트가 동네를 영영 못 바꾸게 된다.**

기본값을 넣는 순간 바꿀 길을 함께 내야 한다는 뜻이라, 둘을 한 커밋에 넣었다.

딸려서 `SearchRegionPrompt`의 게스트 분기가 죽었다. **걷어냈다** —
닿지 않는 갈래를 남겨 두면 다음에 그 파일을 여는 사람이 그 길이 살아 있다고 믿는다.
회원 분기는 남는다. `/search`에는 가드가 없어서 **동네를 안 정한 계정이 주소로 바로 들어오면**
실제로 닿는다(`/`는 온보딩이 막지만 `/search`는 안 막는다).

#### 4. 게스트에게는 "우리 동네"라고 하지 않는다

제목이 "우리 동네 중고거래"인데 보고 있는 곳이 **대신 세워 준 남의 동네**면 어긋난다.
게스트에게는 동네 이름을 그대로 제목에 쓴다(`석관동 중고거래`).

같은 이유로 `GuestRegionSwitcher`는 고른 적이 없을 때 한 줄을 더 적는다 —
"동네를 정하지 않아 이곳을 보여드리고 있어요." 이 말이 없으면 처음 온 사람은
화면이 어떻게 자기 동네를 아는지 몰라 목록을 믿지 못한다.

#### 5. 권하는 것과 보여주는 것은 자리를 다투는 사이가 아니다

예전 `GuestActions`는 **목록이 있어야 할 자리를 통째로** 차지했다. 전환을 노린 배치인데
정작 전환할 이유(물건)를 안 보여주니 반대로 굴렀다.

배너로 내려 목록 위에 얹었다. "시작하기" 버튼은 그대로 둔다 — 없애는 것이 목적이 아니다.
문구는 물건을 본 뒤에 읽히도록 바꿨다("마음에 드는 물건이 있나요? 로그인하면 채팅으로 거래할 수 있어요").

### 덤으로 고친 것

**동네를 확인하는 중에는 목록을 안 그린다.** 예전에는 프로필을 읽는 동안 `regionCode`가
null로 흘러가 "동네를 설정하면…"이 한 번 번쩍였다. 아직 모르는 것을 "없다"로 읽던 자리다.

### 데모 데이터는 더럽혀지지 않는다

게스트에게는 세션이 없어 `auth.uid()`가 null이고, 쓰기 정책이 전부 `auth.uid() = 소유자`를
요구한다. **RLS가 이미 읽기 전용을 강제하고 있다** — 새 정책도 클라이언트 가드도 없다.
시드 재실행 주기도 없다.

다만 **기본 동네 상수가 시드와 묶여 있다.** `npm run seed:story -- --clean`으로 전부 치우면
첫 화면이 다시 빈다. 그 사실을 상수 주석에 적어 뒀다.

### 이번 범위 밖

- **익명 로그인 데모 계정** — 안 했다. 판단 근거는 `backlog.md`에 있다.
- **채팅을 비로그인에게 노출** — 안 한다. 상세 화면의 "로그인하고 채팅하기"로 존재만 알린다.

---

## 시드가 계정 수를 따라간다 — 세 번째 판 (2026-08-20)

`scripts/seedStory.mjs`. 2026-08-13에 넷으로 짜 둔 도구가 **계정이 여섯이 되자 못 돌았다.**

### 무엇이 막혔나

`resolvePeople`이 `data.length !== 4`면 그 자리에서 섰다. 계정을 둘 더 만든 순간
`npm run seed:story`가 "계정이 넷이어야 한다. 지금 6개다"만 내고 끝났다.

넷을 골라 uuid로 넘기면 돌기는 했다. 하지만 그러면 **새로 가입한 둘은 영원히 빈 사람**이다 —
그 계정으로 로그인하면 글도 채팅도 알림도 없는, 앱을 처음 켠 화면이 나온다.
확인하려고 만든 시드가 정작 **확인하고 싶은 자리**(새 사용자)를 못 만들고 있었다.

### 자리를 넉넉히 두고 있는 만큼만 채운다

이야기는 여전히 A·B·C… 자리로 적고 실제 계정은 실행할 때 붙인다. 달라진 것은 **넷으로
못 박지 않는다**는 점이다.

- `PEOPLE_SLOTS`는 열 자리. 온보딩을 마친 순서대로 앉힌다.
- 세울 조건은 **둘 미만**일 때뿐이다. 하나로는 거래가 성립하지 않는다 —
  자기 글에 자기가 채팅을 여는 꼴이 되는데 `posts_buyer_is_not_seller`가 그것을 막는다.
  심다가 죽느니 시작할 때 선다.
- **없는 자리를 가리키는 줄은 그 줄만 건너뛴다**(`onlyPresentPeople`). 세 번째 판은 여섯 자리로
  적혀 있는데, 계정이 넷뿐인 환경에서 E가 쓴 댓글을 그냥 심으면 `people['E']`가 `undefined`라
  그 자리에서 죽는다. **계정 수는 사람이 정하는 것이니 시드가 맞춰야 한다.**
- 찜과 최근 본 글만은 **줄이 아니라 사람 단위로** 덜어낸다. 한 사람이 없다고 나머지 넷의
  관심까지 지울 이유가 없다.

### `--more`가 판 번호가 됐다

판이 셋이 되자 `--more`(두 번째 판)라는 이름이 더는 안 맞았다. `WAVES` 표에 판을 나란히 적고
`--wave 3`으로 부른다. `--more`는 `--wave 2`의 옛 이름으로 그대로 둔다 —
부르는 쪽을 고치게 만들 이유가 없다.

뒤 판이 앞 판의 글에 말을 붙이는 것도 그대로다. 다만 되짚는 대상이 "첫 판"에서
**"앞선 판 전부"**로 넓어졌다(`WAVES.slice(0, waveNumber - 1)`).

### 세 번째 판이 심는 것

글 27 · 댓글 75(비밀 4) · 채팅방 17 · 메시지 74(사진 8) · 후기 8 · 찜 46 · 최근 본 글 28.

새로 들어온 둘을 이야기 한가운데 놓았다. **F는 파는 쪽**(글 아홉 — 모니터·에어프라이어·
운동화·나눔 옷걸이…), **E는 사고 파는 쪽 양쪽**(글 다섯, 채팅 아홉)에 걸친다.
그리고 **앞선 두 판의 글에도** 그 둘이 댓글을 달고 채팅을 연다 — 새로 온 사람이 이미 있던
글을 보고 말을 거는 것이 실제로 벌어지는 일이고, 그 모습이 있어야 오래된 글이 죽어 보이지 않는다.

처음으로 심은 자리도 있다.

- **나눔이 끝나는 이야기** — A의 드립 세트를 E가 받아 가고 `좋은 가격에 나눔해 주셨어요`
  태그가 붙는다. 0원 글이 거래완료까지 가는 길이 이제 데이터에 있다.
- **삽니다 글**(카테고리 84) — A가 애플펜슬을 구한다. 파는 글만 있던 목록에 결이 하나 생겼다.
- **답이 없는 방** — F의 스피커에 B가 한 줄 보내고 끝난다. 읽지 않은 방이 목록에서 어떻게
  보이는지는 그런 방이 있어야 확인된다.

심은 뒤 전체: 글 51(판매중 40 · 예약중 2 · 거래완료 9) · 댓글 110(비밀 16) · 채팅방 27 ·
메시지 122 · 가격제안 13(수락 4 · 대기 5 · 취소 2 · 거절 2) · 후기 14 · 찜 74 · 알림 289.
`npm test` 985개 통과.

### 밟다가 고친 것 둘

**하나. `--clean`이 사람이 올린 프로필 사진까지 지우고 있었다.**

`seedAvatars`는 이미 사진이 있으면 안 덮는다(2026-08-13에 고친 자리). 그런데 `clean`은
`manifest.profileIds` **전부**의 `avatar_url`을 null로 밀고 있었다. 목록에는 심지 않은
사람도 들어 있다 — 넷 중 둘은 자기가 올린 사진이었고, 치우는 순간 그것까지 날아간다.
**안 덮기로 해 놓고 지울 때는 구별하지 않던** 셈이다.

심은 파일 경로가 `<id>/seed-avatar.jpg`라 앞머리가 곧 그 사람이다. 목록의 사람이 아니라
**심은 파일에서 사람을 되짚어** 그만 뗀다. 새 필드를 만들지 않아 이미 쓰던 목록과도 맞는다.

**둘. 사진 한 장이 걸리면 판 전체가 멈췄다.**

한 판이 loremflickr에서 사진 서른아홉 장을 받는다. 남의 서버라 가끔 튕기는데, 그때
`fail()`이 곧바로 서서 **여태 심은 것이 어중간하게 남았다.** 세 번까지 다시 청하고
(1초·2초 쉬며) 그래도 안 되면 그때 선다. 사진 없는 글이 섞이면 화면 버그와
구별되지 않으니 **끝내 실패하면 서는 것은 그대로 둔다.**

### 심기 전에 셈부터 맞춘다

글 27개를 손으로 적었더니 가리키는 곳이 어긋나기 쉬웠다. 심기 전에 데이터만 떼어 훑는
검사를 한 번 돌렸다 — 없는 글을 가리키는 댓글, 파는 사람이 곧 사는 사람인 방,
같은 (글·산 사람) 방이 두 개, 거래완료가 두 번인 글, 거꾸로 흐르는 대화 시각,
`reviews_score_allowed`에 없는 점수. **DB에 닿기 전에 잡아야 하는 것들**이고,
실제로 이 판을 짜는 동안 몇 개를 이 검사가 먼저 잡았다.
