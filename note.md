...
**경로 둘째 칸(`user_id`)의 주인이 `auth.users`에 없는 파일.** 방이 살아 있는지는 묻지 않는다 —
방이 있어도 주인이 없으면 `chat_images_select`가 요구하는 "그 방의 참여자" 자리가 비어
**아무도 못 읽는다.**

살아 있는 계정 목록을 **한 번에 받아 두고** 메모리에서 판단한다. 파일마다 물으면 왕복이
파일 수만큼 든다.
...
- 실행 결과: 방 폴더 1개 · 살아 있는 계정 5개 → 고아 1개 발견 → `--go`로 삭제
- 지운 뒤 **세 버킷 전부 0개**(`chat-images` · `avatars` · `post-images`)

### 남은 반쪽

**주인은 살아 있는데 방만 죽은 사진**은 이 도구가 안 잡는다. A가 탈퇴해 방이 cascade로
사라지면 B의 사진은 아무도 못 읽는데, B는 살아 있으니 "주인 없음" 규칙에 안 걸린다.
`backlog.md` §4의 "회원탈퇴 때 상대가 올린 채팅 사진"이 그것이고,
**이 스크립트에 "방 폴더가 `chat_rooms`에 없으면 쓰레기" 규칙 한 줄을 더하면 닫힌다.**
