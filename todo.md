...
- **설계**: 프로필 페이지·채팅방 ⋯ 메뉴에 차단/해제. **차단은 목록에서 걸러내야 의미가 있다** —
  `search_posts`·`fetch_chat_rooms`에 `blocks` 제외 조건을 넣는 것이
  실제 작업량의 대부분이다. 3단계에서 홈 목록도 `search_posts`로 합쳤으므로 목록 쪽은 한 곳이다. 마이페이지에 차단 목록 관리 화면.
- **마이그레이션**: `0014_block_filter.sql` (기존 RPC들 재정의 — 5단계가 0013을 썼다)
