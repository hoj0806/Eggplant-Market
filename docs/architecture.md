...
`jest.config.cjs`가 **네 갈래(projects)** 로 나뉜다.

| 갈래 | 무엇을 묻나 | 돌리는 법 |
|---|---|---|
| `unit` (jsdom) | 로직이 맞는가 | `npm run test:unit` |
| `integration` (node) | DB·정책이 맞는가 | `npm run test:integration` |
| `build` (node) | **제대로 만들어졌는가** | `npm run test:build` |
| `smoke` (node) | **제대로 서비스되는가** | `npm run test:smoke -- <주소>` |

`npm test`는 **앞의 둘만** 돌린다. 뒤의 둘은 각각 준비물이 필요해서다 —
`test:build`는 `npm run build`부터 하고, `test:smoke`는 주소를 받는다.
섞어 돌리면 **낡은 `dist/`나 없는 주소를 검사한 초록**이 나오는데,
그런 초록은 그물이 없는 것보다 나쁘다(있다고 믿게 만든다).
...
(2026-08-08에 환경변수를 고치고 재배포를 안 해 옛 번들이 계속 나갔다). 그쪽은 아래 `smoke`가 맡는다.

### smoke (node) — `*.smoke.test.ts`

**배포된 주소를 밖에서 두드린다.** `npm run test:smoke -- https://…`

`build`와 겹치는 검사가 있는데 **일부러 그렇다.** `build`는 `dist/`가 옳은지 보고,
`smoke`는 **그 `dist/`가 실제로 올라갔는지** 본다. 2026-08-08에 고친 번들이 안 올라가
옛 것이 계속 서비스됐고, 그때 로컬 검사는 내내 초록이었다.

> **`smoke`가 빨간데 `build`가 초록이면 코드가 아니라 배포가 밀린 것이다.**

- SPA 폴백(`/posts/1` 등) · 정적 파일이 폴백에 안 가로채이는지 · 1년 캐시 헤더
- **배포된 번들**에 환경변수가 박혔는지 · 비밀이 안 섞였는지
  (규칙은 `shared/testUtils/bundlePatterns.ts` 한 곳에 있고 `build`와 나눠 쓴다)
- **카카오 SDK 도메인** — `Referer`를 배포 주소로 바꿔 요청해 200인지.
  등록 안 한 주소로도 한 번 불러 **막히는지**까지 본다. 그래야 위 200이
  "등록됐다"는 뜻이지 "서버가 살아 있다"는 뜻이 아니게 된다.
- `/auth/v1/settings` — `kakao`·`google`만 `true`, `email`은 `false`
- **주소를 안 주면 건너뛰지 않고 실패시킨다.**

**닿지 못하는 곳** 셋.

- **Supabase Redirect URLs.** `/authorize`는 등록 안 한 주소도 그대로 통과시킨다 —
  GoTrue는 **콜백 때** 검사한다. **실제 로그인만이 유일한 검증**이다.
- **Vercel Preview 배포.** 배포 보호가 걸려 모든 요청이 로그인으로 302된다(2026-08-09 확인).
  그래서 잴 수 있는 것은 프로덕션뿐이다.
- **브라우저가 있어야 아는 것** — 캔버스 축소·Realtime 채팅·로그인 플로우·실제 렌더링.
  Playwright를 들이면 되지만 설치·CI·플레이키 관리가 따라오는 별개 작업이라 **하지 않기로 했다.**
  화면 회귀가 실제로 반복될 때 다시 꺼낸다.
