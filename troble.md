...
### 13. 헤드리스 브라우저로 모바일 폭 스크린샷을 못 찍는다

**증상**: `--window-size=390,844`로 찍었더니 **카드가 오른쪽으로 잘려** 나갔다.
반응형이 깨진 줄 알았다.

**원인**: 헤드리스의 **최소 뷰포트가 800px 근처**다. 창은 390으로 잡히지만 레이아웃은
800으로 계산되고, 스크린샷은 그중 왼쪽 390px만 담는다. **앱이 아니라 찍는 쪽 문제였다.**

**해결**: `--window-size=1280,900`으로 찍었다. 데스크탑 레이아웃이 온전히 나온다.
모바일 화면이 필요하면 실제 기기나 브라우저 devtools로 찍어야 한다.

**교훈**: 화면이 이상하면 **찍는 도구를 먼저 의심한다.** 같은 페이지를 다른 폭으로 한 번
찍어 보면 5초에 갈린다.

### 14. 헤드리스 스크린샷이 조용히 실패한다 (그리고 옛 이미지를 준다)

**증상 둘.**

1. 여러 장을 잇달아 찍으면 **첫 장만 생기고 나머지는 파일이 안 생긴다.** 오류도 없다.
2. 새 이미지를 올리고 다시 찍었는데 **옛 이미지가 그대로 나왔다.**

**원인**: 1번은 `--user-data-dir`를 재사용해서다 — 앞 프로세스가 프로필을 잡고 있으면
뒤엣것이 조용히 죽는다. 2번은 Edge 쪽 캐시로 보인다(같은 주소를 Chrome으로 찍으니
새 이미지가 나왔다).

**해결**: `--user-data-dir`를 **매번 새 GUID로** 준다. 그리고 **찍은 것을 눈으로 본다.**

**교훈**: 스크린샷 도구는 **종료코드가 최신 화면을 보장하지 않는다.** 파일 크기가 바뀌어도
내용은 옛것일 수 있다. 결과물을 눈으로 확인하기 전까지는 찍은 것이 아니다.

### 15. `npm test`가 한 번 흔들렸다 (한 스위트 17개)

**증상**: `npm test`가 `Tests: 17 failed, 887 passed`로 끝났다. 어느 스위트인지 출력에서
못 잡았고(`FAIL` 줄을 grep해도 안 나왔다), **바로 다시 돌리니 904개 전부 통과**했다.
이어서 두 번 더 돌려도 통과했다.

**확인한 것**: 유닛과 통합을 **따로** 돌리면 둘 다 통과한다(817 + 87 = 904).
함께 돌릴 때만 났다.

**의심**: 통합 테스트는 **실제 Supabase 프로젝트 하나를 공유**한다. jest가 파일을 병렬로
돌리므로 씨앗·정리가 겹치면 서로를 밟을 수 있다. 각 스위트가 자기 데이터만 지우도록
`fixtures.ts`가 잡혀 있지만, 스토리지 훑기처럼 **버킷 최상위를 통째로 보는 검사**가
새로 생겼다는 점이 달라진 조건이다.

**지금은 안 고친다.** 재현이 안 되고, 원인을 못 짚은 채 병렬을 끄면 느려지기만 한다.
**다시 나면 `--runInBand`로 갈라 보는 것이 첫 걸음**이다.

**교훈**: 흔들린 테스트는 **못 잡았더라도 적어 둔다.** 다음에 같은 일이 났을 때
"처음이 아니다"를 아는 것과 모르는 것은 조사 시간이 다르다.

---

## 통합 테스트가 `JWT issued at future`로 열 개 빨개졌다 (2026-08-10)

### 1. 다시 흔들렸다 — 이번엔 오류 문구가 하나로 모였다

**증상**: `npm test`가 `Tests: 10 failed, 911 passed`로 끝났다. 빨간 것은 전부
`mannerTemp.int.test.ts` 한 스위트였고, 열 개가 **같은 한 줄**을 냈다.
...
**바로 다시 돌리니 통합 87개 전부 통과**했다. 위 15번(`한 스위트 17개`)의 두 번째 발생이다.
그때는 어느 스위트인지도 못 잡았는데, 이번에는 스위트와 문구가 다 남았다.

**원인 찾기**: 문구가 시계를 가리키니 시계부터 쟀다.

```
$ date -u                              Mon, 10 Aug 2026 02:48:37 GMT   ← 이 컴퓨터
$ curl -sI https://supabase.com | grep Date:   ... 02:48:35 GMT        ← 바깥
```

**로컬이 2초 빠르다.** 방향은 맞는다 — 서명한 시각이 검증하는 쪽의 "지금"보다 뒤면
`issued at future`다.

**원인(확정 못 함)**: 다만 **이 2초로 설명이 끝나지는 않는다.** 서비스 키가
`sb_secret_…`(새 형식)이라 **로컬은 JWT를 만들지 않는다** — 게이트웨이가 키를 받아
자기 시계로 JWT를 만들고 PostgREST가 검증한다. 즉 서명도 검증도 서버 쪽이라
로컬 시계가 직접 닿는 자리가 아니다. 남는 설명은 **서버 두 조각 사이의 순간적인 어긋남**인데
밖에서 잴 방법이 없다.

**해결**: 안 고쳤다. **다시 돌리면 통과한다.** 15번과 달리 이번엔 두 가지가 남았다.

- 이 흔들림은 **경합이 아니라 시계**다. 15번에서 의심한 "통합이 DB 하나를 공유해서"는
  적어도 이번 건의 원인이 아니다 — 같은 오류 문구 열 줄은 데이터가 겹칠 때 나오는 모양이 아니다.
- `--runInBand`로 갈라 볼 이유도 줄었다. 다음에 또 나면 **`date`부터 재고 문구를 그대로 적는다.**

**교훈**: 흔들리는 테스트는 **오류 문구가 열 개 다 같은지 제각각인지**부터 본다.
같으면 바깥 사정(시계·네트워크·쿼터)이고, 제각각이면 안쪽 사정(경합·순서)이다.
15번에서 "어느 스위트인지 못 잡았다"고만 적어 둔 탓에 이번에 처음부터 다시 좁혀야 했다.


### 2. 번들 해시로 "배포됐나"를 재려다 3분을 헛돌았다

**증상**: `main`에 머지하고 Vercel이 새 번들을 서비스하는지 보려고, 로컬 빌드가 낸 파일 이름
(`assets/index-7o4MVA-r.js`)이 배포된 `index.html`에 나타나기를 기다렸다. **20번을 찔러도
계속 `index-Bo18DTDh.js`** 였다. 배포가 안 됐거나 빌드가 깨진 줄 알았다.

**원인**: **Vercel은 자기 환경변수로 자기가 빌드한다.** 자산 파일명은 내용 해시이고 그 내용에는
`import.meta.env`로 박히는 값이 들어간다 — 로컬 `dist`와 배포본은 **애초에 같은 이름이 될
이유가 없다.** 기다리던 이름은 영원히 안 온다.

**해결**: 이름 대신 **내용**을 봤다. 이번 배포에만 있는 문자열로 가른다.

```
$ curl -s $URL/assets/index-Bo18DTDh.js > served.js
  "모두 삭제"                    1   ← 새 코드에만 있다
  "모두 읽음"                    0   ← 옛 코드에만 있다
  count_unread_notifications     0   ← 옛 코드에만 있다
```

셋 다 새 번들의 모양이었다. **이미 배포돼 있었고 3분은 통째로 헛걸음이었다.**

**교훈**: 배포 여부는 **바뀐 내용으로** 잰다. 파일 이름·크기·해시는 빌드하는 기계가 다르면
같이 안 간다. `test:smoke`가 처음부터 그렇게 만들어져 있었다(번들에서 **값의 형식**을 찾지
파일명을 보지 않는다) — 정작 그 그물을 만들어 놓고 손으로는 옛 방식으로 쟀다.

---

## 고쳤다고 적어 둔 버그가 그대로 살아 있었다 (2026-08-10)

### 1. 코드는 머지됐는데 **Edge Function이 배포되지 않았다**

**증상**: 카카오·구글 두 계정으로 사진을 주고받고 **둘 다 탈퇴**했다. DB는 깨끗했는데
`chat-images`에 **파일 하나가 남았다.**

```
442/ce3043d8-…/1786336229843-0.png     312 kB
  주인(ce3043d8) → auth.users에 없음
  방(442)        → chat_rooms에 없음
```

**이 모양은 이미 아는 버그다.** `backlog.md` §1-3 "방이 먼저 사라진 채팅 사진" —
첫 번째 탈퇴가 방을 cascade로 가져가면, 두 번째 사람은 `fetchRoomIds()`가 빈 배열이라
**자기 사진인데도** 지울 경로를 모른다. 2026-08-09에 버킷 훑기(`chatImageSweep.ts`)로
고쳤고 PR #61로 머지까지 했다.

**원인 찾기**: 로컬 `index.ts`는 분명히 `sweepOrphanChatImages`를 부르고 있었다.
그래서 **배포된 쪽**을 열어 봤다.

```
list_edge_functions  →  delete-account  version: 2
배포된 index.ts      →  sweepOrphanChatImages 없음. fetchRoomIds 갈래만 있다.
```

**원인**: **배포를 안 했다.** 그날 노트에도 `backlog.md`에도
"`supabase functions deploy delete-account`가 필요하다"고 적어 두었는데, 적어 두기만 했다.
머지는 코드를 `develop`에 넣을 뿐 **Edge Function은 따로 배포해야 한다** —
프런트엔드는 Vercel이 `main`을 보고 자동으로 나가지만 함수는 아무도 안 밀어 준다.

**해결**
1. v3로 배포했다(MCP `deploy_edge_function`). 배포본에 sweep이 들어간 것을 다시 읽어 확인했다.
2. 이미 생긴 고아 파일은 **아무도 못 지운다** — 주인이 없으니 그 사람의 탈퇴가 다시 일어날 일이
   없고, 방도 없어 정책이 볼 근거가 없다. 그래서 `scripts/sweepOrphanChatImages.mjs`를 만들어
   치웠다(찾기가 기본, `--go`로 지움). 지금 **세 버킷이 전부 0개**다.

**교훈 셋**
- **"고쳤다"와 "돌고 있다"는 다르다.** 이 저장소는 그 차이를 이미 두 번 겪었다 —
  8월 9일 환경변수(고치고 재배포를 안 함), 오늘 Edge Function. 둘 다 **테스트는 초록이었다.**
- **자동으로 안 나가는 것에는 그물이 없다.** `test:build`·`test:smoke`가 프런트엔드는 지키지만
  Edge Function 버전은 아무도 안 본다. 배포가 필요한 변경은 **머지와 같은 자리에서 끝내야 한다.**
- 뒷정리 코드는 **끊길 수 있다**는 전제가 맞았다. 그래서 사람이 돌리는 청소 도구가 하나 필요했고,
  그것이 `backlog.md`에 "끊긴 뒷정리 다시 시도하기"로 적혀 있던 부채였다.

### 3. 통합 테스트가 계정을 흘리고 있었다 — 등록이 한 줄 늦어서

**증상**: `auth.users`에 `int-test-*@eggplant.test` 계정이 **넷** 쌓여 있었다.
픽스처는 "심은 것만 치운다"고 만들어 두었는데 치워지지 않은 것들이다.

**단서는 넷이 똑같이 생겼다는 것**이었다.

```
nickname      user_98daa538   ← handle_new_user 트리거가 만드는 기본값
onboarded_at  null            ← createTestUser의 update가 닿지 않았다
posts         0
```

**원인**: `createTestUser`의 순서다.

```ts
const { data } = await admin.auth.admin.createUser(...)   // ① 계정이 생긴다
...
if (profileError !== null) throw profileError             // ② 여기서 던지면
const user = { id: userId, ... }
fixture.users.push(user)                                  // ③ 여기에 못 온다
```

②에서 던지는 순간 **계정은 이미 있는데 픽스처는 그 존재를 모른다.** `cleanup()`이 도는
`afterAll`은 정상적으로 실행되지만 지울 목록에 그 계정이 없다 — **지울 수 없는 유령**이다.

②를 실제로 터뜨린 것은 위 1번의 시계 문제였다. GoTrue의 admin API(①)는 통과하고
PostgREST(②)만 `JWT issued at future`로 거절당하니, **정확히 이 틈이 벌어진다.**

**해결 둘**
1. **만들자마자 등록한다.** ③을 ① 바로 뒤로 옮겼다. 뒤에 무엇이 실패해도 치울 수 있다.
2. **cleanup이 실패를 삼키지 않게 했다.** 전에는 지우기 결과를 안 보고 목록을 비웠다 —
   못 지운 계정이 있어도 아무도 몰랐다. 이제 못 지운 것은 목록에 남기고 **예외를 던진다.**
   앱의 뒷정리는 조용히 넘어가는 것이 맞지만(사용자가 할 수 있는 일이 없다) **여기는 반대다** —
   읽는 사람이 개발자이고 지금 고칠 수 있다.

**바로 확인됐다.** 고친 뒤 돌린 `npm test`에서 **같은 흔들림이 또 났는데**(10개 실패)
이번에는 `int-test-*` 계정이 **0개**였다. 고치기 전이라면 하나가 남았을 자리다.

**교훈**: **"만들었다"와 "치울 수 있다" 사이를 벌리지 않는다.** 되돌릴 대상은 만들어진
그 줄에서 등록한다. 그 사이에 무엇이든 끼면, 그 무엇이 실패하는 날 쓰레기가 남는다.


---

## 시드가 계정 수에 못 박혀 있었다 (2026-08-20)

### 1. 계정을 둘 더 만들었더니 시드가 아예 안 돌았다

**증상**: `npm run seed:story`가 한 줄만 내고 끝났다.

```
계정이 넷이어야 한다. 지금 6개다.
```

**원인**: `resolvePeople`이 `data.length !== 4`로 세우고 있었다. 넷일 때 짠 도구라
**넷이 아니면 많아도 선다.** 이야기 자리도 `['A','B','C','D']`로 못 박혀 있어
다섯째부터는 앉을 자리 자체가 없었다.

넷을 uuid로 골라 넘기면 돌기는 했다. 하지만 그러면 **새로 가입한 둘은 계속 빈 사람**이다 —
그 계정으로 로그인하면 앱을 처음 켠 화면이 나온다. 확인하려고 만든 시드가 정작
**가장 확인하고 싶은 자리**를 못 만들고 있었다.

**해결**: 자리를 열 개(`PEOPLE_SLOTS`) 두고 **있는 만큼만** 앉힌다. 세우는 조건은
**둘 미만**일 때뿐이다 — 하나로는 거래가 성립하지 않는다(`posts_buyer_is_not_seller`).
그리고 **없는 자리를 가리키는 줄은 그 줄만 건너뛴다**(`onlyPresentPeople`).
계정이 넷인 환경에서도 여섯 자리로 적은 판이 그대로 돈다.

**교훈**: **지금 몇인지를 코드에 적지 않는다.** "넷"은 그날의 사실이었지 규칙이 아니었다.
진짜 규칙은 "거래에는 둘이 필요하다"뿐이고, 그것만 검사하면 나머지는 늘어나도 안 깨진다.

### 2. `--clean`이 사람이 올린 프로필 사진까지 지우고 있었다

**증상**: 눈에 띄기 전에 코드에서 먼저 잡았다. `--clean`을 돌리면 시드가 **심지 않은**
프로필 사진까지 사라진다.

**원인**: `seedAvatars`는 이미 사진이 있으면 안 덮는다(2026-08-13에 고친 자리).
그런데 `clean`은 `manifest.profileIds` **전부**의 `avatar_url`을 null로 밀었다.
그 목록은 "심은 사람"이 아니라 "이번 판에 낀 사람"이다 — 넷 중 둘은 자기가 올린 사진이었다.
**안 덮기로 해 놓고 지울 때는 구별하지 않던** 셈이다.

**해결**: 심은 파일 경로가 `<id>/seed-avatar.jpg`라 앞머리가 곧 그 사람이다.
목록의 사람이 아니라 **심은 파일에서 사람을 되짚어** 그만 뗀다. 새 필드가 없어
이미 쓰던 `.seedStory.json`과도 그대로 맞는다.

**교훈**: **만드는 쪽에 둔 예외는 지우는 쪽에도 있어야 한다.** "안 덮는다"와 "안 지운다"는
한 쌍인데, 한쪽만 고치면 반쪽짜리 규칙이 조용히 남는다.

### 3. 사진 한 장이 걸리면 판 전체가 멈췄다

**증상**: 한 판이 loremflickr에서 사진 서른아홉 장을 받는데, 그중 한 장이 502를 받으면
`fail()`이 그 자리에서 서고 **여태 심은 것이 어중간하게 남았다.**

**원인**: 남의 서버가 가끔 튕기는 것을 **영구 실패로 다뤘다.**

**해결**: 세 번까지 다시 청한다(1초·2초 쉬며). 세 번 다 실패하면 그때 선다 —
사진 없는 글이 섞이면 화면 버그와 구별되지 않으니 **끝내 실패하면 서는 것은 그대로 둔다.**

**교훈**: 다시 해보면 되는 실패와 다시 해도 안 되는 실패를 같은 손으로 다루지 않는다.
